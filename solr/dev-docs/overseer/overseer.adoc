...
. A former overseer might be overwriting cluster state once it is no longer Overseer. #bookmark=id.m1okfcnai3fd[In section ClusterStateUpdater]
. Efficiency of reading the Collection queue decreases as the queue grows, when things go bad they can get worse. #bookmark=id.bonu9bb600ui[In OveerseerTaskQueue section]. Reported as a real bug, see https://lucene.472066.n3.nabble.com/Overseer-documentation-tp4457133p4457251.html[mail].
. Config set locking race condition. #bookmark=id.gm592ko784i6[In OverseerConfigSetMessageHandler section].
. Lock on base config set released too early. #bookmark=id.id9bn1f4hyl6[In OverseerConfigSetMessageHandler section].
. LockTree Session management is incorrect, Sessions likely useless. #bookmark=id.oh0pwq90n27s[In section Collection API lock usage].
. It is possible to acquire conflicting locks in collection processing. #bookmark=id.6f042vlsnph9[In section Collection API lock usage].
. Collection message handler locking is done in an inefficient way. #bookmark=id.rcipcvk38wbv[In section Proposal to fix the infra?]
. Timing out collection API commands lead to orphaned state change messages that could lead to inconsistencies. #bookmark=id.gazj6pvrxy8a[In section A deeper issue with the locking strategy?]  Reported as a real bug, see https://lucene.472066.n3.nabble.com/Overseer-documentation-tp4457133p4457251.html[mail].
. Requesting replica creation one by one with ADDREPLICA in the state update queue is inefficient. #bookmark=id.f4ks7mwo0dpa[In CreateCollectionCmd section]
. Replica placement on the cluster can be unbalanced under high load. #bookmark=id.a74ab6er8nih[In CreateCollectionCmd section]. See https://issues.apache.org/jira/projects/SOLR/issues/SOLR-14462[SOLR-14462].
. Under specific conditions, dequeue from the collection queue can slow down to a message every two seconds. #bookmark=id.igue2gy2wavc[In OverseerTaskProcessor section]
. Passing bad parameters when setting shard leader can lead to shard having no leader. #bookmark=id.j0uwimlbgl0o[In SliceMutator section]
. Inefficiency in how DOWNNODE overseer action is handled. #bookmark=id.sfc6mib8xy6l[In NodeMutator section]
