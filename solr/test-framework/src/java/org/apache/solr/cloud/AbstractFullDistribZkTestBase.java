  ...
  protected List<JettySolrRunner> createJettys(int numJettys) throws Exception {
    ...
    for (int i = 1; i <= numJettys; i++) {
      ...
      if (numOtherReplicas > 0) {
        numOtherReplicas--;
        if (useTlogReplicas()) {
          int tmp = ((currentI % sliceCount) + 1)
          log.info("create jetty {} in directory {} of type {} in shard {}", i, jettyDir, Replica.Type.TLOG, tmp);
          ...
          int tmp = ((currentI % sliceCount) + 1);
          log.info("create jetty {} in directory {} of type {} for shard{}", i, jettyDir, Replica.Type.NRT, tmp);
        ...
        int tmp = ((currentI % sliceCount) + 1);
        log.info("create jetty {} in directory {} of type {} for shard{}", i, jettyDir, Replica.Type.PULL, tmp);
        customThreadPool.submit(() -> {
          try {
            JettySolrRunner j = createJetty(jettyDir, useJettyDataDir ? getDataDir(testDir + "/jetty"
 ...
 public static void waitForNon403or404or503(HttpSolrClient collectionClient)
  ...
  protected List<Replica> ensureAllReplicasAreActive(String testCollectionName, String shardId, int shards, int rf, int maxWaitSecs) throws Exception {
    ...
    while (waitMs < maxWaitMs && !allReplicasUp) {
      ...
      if (log.isInfoEnabled()) {
        log.info("Found {} replicas and leader on {} for {} in {}", replicas.size(), leader.getNodeName(), shardId, testCollectionName);
      }
