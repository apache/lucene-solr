...
// 5
    ...
    assertJQ(req("q", "{!join from=" + idField + " to=" + toField + " score=None}name:name2", "fl", "id")
        , "/response=={'numFound':2,'start':0,'hitCountRelation':true,'docs':[{'id':'5'},{'id':'6'}]}");
    ...
    */
    assertJQ(req("q", "{!join from=" + idField + " to=" + toField + " score=None}name:name1", "fl", "id")
        , "/response=={'numFound':2,'start':0,'hitCountRelation':true,'docs':[{'id':'2'},{'id':'3'}]}");
    ...
    assertJQ(req("q", "{!join from=" + toField + " to=" + idField + " score=None}id:5", "fl", "id")
        , "/response=={'numFound':1,'start':0,'hitCountRelation':true,'docs':[{'id':'4'}]}");
  ...
  public void testDeleteByScoreJoinQuery() throws Exception {
    ...
    assertJQ(req("q", joinQuery, "fl", "id"), "/response=={'numFound':2,'start':0,'hitCountRelation':true,'docs':[{'id':'1'},{'id':'4'}]}");
    assertU(delQ(joinQuery));
    assertU(commit());
    assertJQ(req("q", joinQuery, "fl", "id"), "/response=={'numFound':0,'start':0,'hitCountRelation':true,'docs':[]}");
  }
  ...
  public void testSimpleWithScoring() throws Exception {
    ...
    assertJQ(req("q", "{!join from=" + toField + " to=" + idField + " score=Max}title:random", "fl", "id")
        , "/response=={'numFound':2,'start':0,'hitCountRelation':true,'docs':[{'id':'1'},{'id':'4'}]}");
    ...
    assertJQ(req("q", "{!join from=" + toField + " to=" + idField + " score=Max}title:movie", "fl", "id")
        , "/response=={'numFound':2,'start':0,'hitCountRelation':true,'docs':[{'id':'4'},{'id':'1'}]}");
