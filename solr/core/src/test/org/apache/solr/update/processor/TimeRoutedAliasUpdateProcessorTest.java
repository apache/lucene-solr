...
public class TimeRoutedAliasUpdateProcessorTest extends SolrCloudTestCase {
  ...
  public void test() throws Exception {
    ...
    // We configure it, and ultimately delete it the collection, leaving a modified config-set behind.
    ...
    assertTrue(
        new ConfigSetAdminRequest.List().process(solrClient).getConfigSets()
            .contains(configNameOfModifiedConfigSet)
    );

    // start with one collection and an alias for it
    final String col23rd = alias + "_2017-10-23";
    CollectionAdminRequest.createCollection(col23rd, configNameOfModifiedConfigSet, 1, 1)
        .withProperty(TimeRoutedAliasUpdateProcessor.TIME_PARTITION_ALIAS_NAME_CORE_PROP, alias)
        .process(solrClient);

    List<String> retrievedConfigSetNames = new ConfigSetAdminRequest.List().process(solrClient).getConfigSets();
    List<String> expectedConfigSetNames = Arrays.asList("_default", configName, configNameOfModifiedConfigSet );
    ...
    CollectionAdminRequest.createCollection(col24th, configNameOfModifiedConfigSet,  2, 2) // more shards and replicas now
