...
public class DistributedUpdateProcessorTest extends SolrTestCaseJ4 {
  ...
  public static void beforeClass() throws Exception {
    ...
    lock.lock();
    try {
      lock.newCondition().await(-1, TimeUnit.MILLISECONDS);
      lock.newCondition().await(0, TimeUnit.MILLISECONDS);
    } finally {
      lock.unlock();
    }
  ...
  public int runCommands(int threads, int verionLockInMill, SolrQueryRequest req, Function<DistributedUpdateProcessor,Boolean> function)
      throws IOException {
    DistributedUpdateProcessor processor = new DistributedUpdateProcessor(
        req, null, null, null) {
      @Override
      protected boolean tryGetVersionBucketLock(VersionBucket bucket) {
        bucket.setVerionLockInMill(verionLockInMill);

        boolean locked = super.tryGetVersionBucketLock(bucket);
        if (locked) {
          try {
          ...
          throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
              "Unable to get version bucket lock in " + bucket.getVerionLockInMill() + " mill seconds");
        }
        return locked;
      }
    };
