...
public class DeleteReplicaTest extends SolrCloudTestCase {
  ...
  public void deleteReplicaFromClusterState() throws Exception {
    ...
    if (replicaZkController.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
      cluster.getOpenOverseer().getDistributedClusterChangeUpdater().doSingleStateUpdate(
          DistributedClusterChangeUpdater.MutatingCommand.SliceRemoveReplica, m,
    ...
    ZkContainer.testing_beforeRegisterInZk = cd -> {
      ...
      if (replica1.getName().equals(cd.getCloudDescriptor().getCoreNodeName())
        ...
        try {
          ...
          if (replica1ZkController.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
            cluster.getOpenOverseer().getDistributedClusterChangeUpdater().doSingleStateUpdate(
                DistributedClusterChangeUpdater.MutatingCommand.SliceRemoveReplica, m,
                cluster.getOpenOverseer().getSolrCloudManager(),
                cluster.getOpenOverseer().getZkStateReader());
