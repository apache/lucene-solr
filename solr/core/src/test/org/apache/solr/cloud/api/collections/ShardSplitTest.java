...
public class ShardSplitTest extends BasicDistributedZkTest {
    ...
    try (CloudSolrClient client = getCloudSolrClient(zkServer.getZkAddress(), true, cloudClient.getLbClient().getHttpClient())) {
      ...
      try {
        ...
        if (state == RequestStatusState.COMPLETED)  {
          ...
          try (Http2SolrClient control = new Http2SolrClient.Builder(control_collection).withHttpClient(client.getLbClient().getHttpClient()).build())  {
    ...
    for (Replica replica : shard.getReplicas()) {
      try (Http2SolrClient client = new Http2SolrClient.Builder(replica.getCoreUrl())
        ...
        if (numFound == Long.MIN_VALUE) {
          ...
          assertEquals("Shard " + shard.getName() + " replicas do not have same number of documents", numFound,
  ...
  private boolean doesReplicaCoreExist(Replica replica) throws IOException {
    try (Http2SolrClient client = new Http2SolrClient.Builder(replica.getStr(BASE_URL_PROP))
        .withHttpClient(cloudClient.getLbClient().getHttpClient()).build())  {
  ...
  public void splitByRouteFieldTest() throws Exception  {
    ...
    try (Http2SolrClient collectionClient = getHttpSolrClient(url)) {
  ...
  protected void splitShard(String collection, String shardId, List<DocRouter.Range> subRanges, String splitKey) throws SolrServerException, IOException {
    ...
    try (Http2SolrClient baseServer = getHttpSolrClient(baseUrl, 30000, 60000 * 5)) {
  ...
  protected SolrClient createNewSolrClient(String collection, String baseUrl) {
    Http2SolrClient client = (Http2SolrClient) super.createNewSolrClient(collection, baseUrl, DEFAULT_CONNECTION_TIMEOUT, 5 * 60 * 1000);
    return client;
  }

  @Override
  protected SolrClient createNewSolrClient(int port) {
    Http2SolrClient client = (Http2SolrClient) super.createNewSolrClient(port, DEFAULT_CONNECTION_TIMEOUT, 5 * 60 * 1000);
    return client;
  }
