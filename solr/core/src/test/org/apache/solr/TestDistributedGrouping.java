...
public class TestDistributedGrouping extends BaseDistributedSearchTestCase {
  ...
  public void test() throws Exception {
    ...
    // SOLR-6203 sorting on user-defined field
    {
      // abs(sub(7,id)) is the user-defined sort field, can be ascending or descending
      // (ascending or descending) score may or may not be included
      // id can be ascending or descending
      String groupSort = "abs(sub(7,id)) "+(random().nextBoolean() ? "asc" : "desc");
      if (random().nextBoolean()) {
        groupSort += ", score desc";
      }
      groupSort += (", id "+(random().nextBoolean() ? "asc" : "desc"));
      query("q", "*:*", "rows", 100, "fl", "id," + i1,
          "group", "true", "group.field", i1, "group.limit", 10,
          "sort", i1 + " asc, abs(sub(5,id)) asc, id asc",
