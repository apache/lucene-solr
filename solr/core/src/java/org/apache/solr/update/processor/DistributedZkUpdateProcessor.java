...
public class DistributedZkUpdateProcessor extends DistributedUpdateProcessor {
  ...
  public DistributedZkUpdateProcessor(
      ...
      SolrQueryResponse rsp, AtomicUpdateDocumentMerger docMerger, CoreContainer cc, UpdateRequestProcessor next) {
  ...
  @Override
  protected SolrInputDocument cloneCmdDoc(AddUpdateCommand cmd){
    final boolean willDistrib = isLeader && nodes != null && nodes.size() > 0;

    if (willDistrib && cloneRequiredOnLeader) {
      return cmd.solrDoc.deepCopy();
    }
    return null;
  }
  ...
  @Override
  public void doDeleteById(DeleteUpdateCommand cmd) throws IOException {
  ...
  public void doDeleteByQuery(DeleteUpdateCommand cmd) throws IOException {
    ...
    if (DistribPhase.NONE == phase) {
      ...
      ModifiableSolrParams outParams = new ModifiableSolrParams(filterParams(getReq().getParams()));
      outParams.set(DISTRIB_UPDATE_PARAM, DistribPhase.TOLEADER.toString());
      outParams.set(DISTRIB_FROM, ZkCoreNodeProps.getCoreUrl(
          zkController.getBaseUrl(), getReq().getCore().getName()));

      SolrParams params = getReq().getParams();
      ...
      if (params.get(UpdateRequest.MIN_REPFACT) != null) {
        ...
        outParams.add(UpdateRequest.MIN_REPFACT, getReq().getParams().get(UpdateRequest.MIN_REPFACT));
      }
  ...
  @Override
  protected void postProcessDeleteByQuery(DeleteUpdateCommand cmd, List<SolrCmdDistributor.Node> replicaNodes, DocCollection docCollection) throws IOException {
    // at this point, there is an update we need to try and apply.
    // we may or may not be the leader.

    final boolean isReplayOrPeersync = (cmd.getFlags() & (UpdateCommand.REPLAY | UpdateCommand.PEER_SYNC)) != 0;
    final boolean leaderLogic = isLeader && !isReplayOrPeersync;
    // forward to all replicas
    ModifiableSolrParams params = new ModifiableSolrParams(filterParams(getReq().getParams()));
    ...
    try {
      final boolean subShardLeader = amISubShardLeader(docCollection, null, null, null);
      if (subShardLeader)  {
    ...
    if (leaderLogic) {
      List<SolrCmdDistributor.Node> subShardLeaders = getSubShardLeaders(docCollection, cloudDesc.getShardId(), null, null);
      if (subShardLeaders != null)  {
        cmdDistrib.distribDelete(cmd, subShardLeaders, params, true, rollupReplicationTracker, leaderReplicationTracker);
      }
      final List<SolrCmdDistributor.Node> nodesByRoutingRules = getNodesByRoutingRules(zkController.getClusterState(), docCollection, null, null);
      if (nodesByRoutingRules != null && !nodesByRoutingRules.isEmpty())  {
        params = new ModifiableSolrParams(filterParams(getReq().getParams()));
        params.set(DISTRIB_UPDATE_PARAM, DistribPhase.FROMLEADER.toString());
        params.set(DISTRIB_FROM, ZkCoreNodeProps.getCoreUrl(
            zkController.getBaseUrl(), getReq().getCore().getName()));
      ...
      if (replicaNodes != null) {
        cmdDistrib.distribDelete(cmd, replicaNodes, params, false, rollupReplicationTracker, leaderReplicationTracker);
        someReplicas = true;
      }
    }
  ...
  protected void postProcessDeleteById(DeleteUpdateCommand cmd) throws IOException {
    ...
    if (nodes != null) {
      ...
      if (getReq().getParams().get(UpdateRequest.MIN_REPFACT) != null) {
        // TODO: Kept for rolling upgrades only. Remove in Solr 9
        params.add(UpdateRequest.MIN_REPFACT, getReq().getParams().get(UpdateRequest.MIN_REPFACT));
      }
  ...
  protected void postProcessAdd(AddUpdateCommand cmd) throws IOException {
    ...
    if (nodes != null) {
      ...
      if (getReq().getParams().get(UpdateRequest.MIN_REPFACT) != null) {
        ...
        params.set(UpdateRequest.MIN_REPFACT, getReq().getParams().get(UpdateRequest.MIN_REPFACT));
      }
  ...
  private List<SolrCmdDistributor.Node> setupRequest(String id, SolrInputDocument doc, String route) {
    ...
    if ((getUpdateCommand().getFlags() & (UpdateCommand.REPLAY | UpdateCommand.PEER_SYNC)) != 0) {
    ...
    Slice slice = coll.getRouter().getTargetSlice(id, doc, route, getReq().getParams(), coll);
    ...
    DistribPhase phase =
        DistribPhase.parseParam(getReq().getParams().get(DISTRIB_UPDATE_PARAM));
    ...
    try {
      ...
      String fromCollection = getUpdateCommand().getReq().getParams().get(DISTRIB_FROM_COLLECTION);
      ...
      if (DistribPhase.FROMLEADER == phase && !isSubShardLeader && fromCollection == null) {
        ...
        String[] skipList = getReq().getParams().getParams(TEST_DISTRIB_SKIP_SERVERS);
    ...
    if (state == Slice.State.CONSTRUCTION || state == Slice.State.RECOVERY) {
      ...
      if (amILeader) {
        ...
        if (parentSlice != null)  {
          ...
          return isSubset && coll.getRouter().isTargetSlice(id, doc, getReq().getParams(), myShardId, coll);
        } else  {
  ...
  private void doDefensiveChecks(DistribPhase phase) {
    boolean isReplayOrPeersync = (getUpdateCommand().getFlags() & (UpdateCommand.REPLAY | UpdateCommand.PEER_SYNC)) != 0;
    if (isReplayOrPeersync) return;

    String from = getReq().getParams().get(DISTRIB_FROM);
    ...
    if (DistribPhase.FROMLEADER == phase && localIsLeader && from != null) { // from will be null on log replay
      String fromShard = getReq().getParams().get(DISTRIB_FROM_PARENT);
      if (fromShard != null) {
        ...
        String fromCollection = getReq().getParams().get(DISTRIB_FROM_COLLECTION); // is it because of a routing rule?
        if (fromCollection == null)  {
          log.error("Request says it is coming from leader, but we are the leader: " + getReq().getParamString());
    ...
    if ((isLeader && !localIsLeader) || (isSubShardLeader && !localIsLeader)) {
      ...
      throw new SolrException(SolrException.ErrorCode.SERVICE_UNAVAILABLE,
          "ClusterState says we are the leader (" + zkController.getBaseUrl()
              + "/" + getReq().getCore().getName() + "), but locally we don't think so. Request came from " + from);
    }
  }
  ...
  public void processCommit(CommitUpdateCommand cmd) throws IOException {
    ...
    List<SolrCmdDistributor.Node> nodes = getCollectionUrls(collection, EnumSet.of(Replica.Type.TLOG, Replica.Type.NRT));
    if (nodes == null) {
    ...
    if (getReq().getParams().getBool(COMMIT_END_POINT, false) || singleLeader) {
      if (replicaType == Replica.Type.TLOG) {
        try {
          ...
          isLeader = leaderReplica.getName().equals(cloudDesc.getCoreNodeName());
          if (isLeader) {
            setVersionAndDoLocalCommit(cmd);
          } else {
            assert TestInjection.waitForInSyncWithLeader(getReq().getCore(),
                zkController, collection, cloudDesc.getShardId()) : "Core " + getReq().getCore() + " not in sync with leader";
          }
        } catch (InterruptedException e) {
      ...
      ModifiableSolrParams params = new ModifiableSolrParams(filterParams(getReq().getParams()));
      if (!getReq().getParams().getBool(COMMIT_END_POINT, false)) {
  ...
  private void zkCheck() {
    ...
    if (getReq().getCore().getCoreContainer().isShutDown()) {
  ...
  private List<SolrCmdDistributor.Node> getCollectionUrls(String collection, EnumSet<Replica.Type> types) {
    ClusterState clusterState = zkController.getClusterState();
      ...
      if (state == Slice.State.CONSTRUCTION || state == Slice.State.RECOVERY)  {
        ...
        if (isSubset &&
            (docId == null // in case of deletes
                || coll.getRouter().isTargetSlice(docId, doc, getReq().getParams(), aslice.getName(), coll))) {
        ...
        if (routeKey != null) {
          ...
          if (rule != null) {
            if (! rule.isExpired()) {
              ...
              ReentrantLock ruleExpiryLock = getReq().getCore().getRuleExpiryLock();
  ...
  private void checkReplicationTracker(UpdateCommand cmd) {
    ...
    if (isLeader && leaderReplicationTracker == null) {
      leaderReplicationTracker = new LeaderRequestReplicationTracker(
          getReq().getCore().getCoreDescriptor().getCloudDescriptor().getShardId());
    }
  }
  ...
  protected void handleReplicationFactor() {
    if (leaderReplicationTracker != null || rollupReplicationTracker != null) {
      ...
      if (getReq().getParams().get(UpdateRequest.MIN_REPFACT) != null) {
        ...
        getRsp().getResponseHeader().add(UpdateRequest.MIN_REPFACT, Integer.parseInt(getReq().getParams().get(UpdateRequest.MIN_REPFACT)));
      }
      getRsp().getResponseHeader().add(UpdateRequest.REPFACT, achievedRf);
      rollupReplicationTracker = null;
  ...
  private void setCloudDescIfNull() {
    if(cloudDesc == null) {
      cloudDesc = getReq().getCore().getCoreDescriptor().getCloudDescriptor();
    }
  }
  ...
  protected Replica.Type parseReplicaType(CoreContainer cc) {
    setCloudDescIfNull();
    return cloudDesc==null? Replica.Type.NRT: cloudDesc.getReplicaType();
  }
