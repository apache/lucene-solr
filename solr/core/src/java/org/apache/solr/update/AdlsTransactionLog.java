 ...
 *
 */
public class AdlsTransactionLog extends TransactionLog {
  ...
  AdlsTransactionLog(AdlsProvider provider,String tlogFile, Collection<String> globalStrings, boolean openExisting) {
    super();
    ...
    try {
      ...
      if (provider.checkExists(tlogFile) && openExisting) {
        start=provider.getDirectoryEntry(tlogFile).length;
        tlogOutStream=provider.getAppendStream(tlogFile);
      } else {
        provider.delete(tlogFile);
        tlogOutStream=provider.createFile(tlogFile, IfExists.FAIL);
      }
      ...
      if (openExisting) {
        ...
        if (start > 0) {
          long size=provider.getDirectoryEntry(tlogFile).length;
          log.error("New transaction log already exists:" + tlogFile + " size=" + size);
        }
  ...
  public boolean endsWithCommit() throws IOException {
    ...
    try (ADLFileInputStream dis = provider.getReadStream(tlogFile);){
      dis.seek(pos);
      dis.read(buf);
  ...
  private void readHeader() throws IOException {
    ...
    Map header = null;
    try (ADLFileInputStream fin = provider.getReadStream(this.tlogFile)) {
      try (LogCodec codec = new LogCodec(resolver)) {
        header = (Map) codec.unmarshal(fin);
      }
    }
  ...
  public void close() {
    try {
      ...
      if (deleteOnClose) {
        try {
          provider.deleteRecursive(tlogFile);
        } catch (IOException e) {
          throw new SolrException(SolrException.ErrorCode.SERVER_ERROR, e);
        }
      }
  ...
  public String toString() {
    return "ADLS tlog{file=" + tlogFile.toString() + " refcount=" + refcount.get() + "}";
  }
  ...
  public LogReader getReader(long startingPos) {
    return new AdlsLogReader(startingPos);
  }

  public LogReader getSortedReader(long startingPos) {
    return new AdlsSortedLogReader(startingPos);
  }

  /** Returns a single threaded reverse reader */
  @Override
  public ReverseReader getReverseReader() throws IOException {
    return new AdlsReverseReader();
  }


  public class AdlsLogReader extends LogReader{
    AdlsDataFastInputStream fis;
    private LogCodec codec = new LogCodec(resolver);
    private long sz;

    public AdlsLogReader(long startingPos) {
      super();
      incref();
    ...
    private void initStream(long pos) {
      try {
        synchronized (AdlsLogReader.this) {
          ensureFlushed();
          sz = getLogSize();
        }
        fis = new AdlsDataFastInputStream(provider.getReadStream(tlogFile), pos);
      } catch (IOException e) {
    ...
    public Object next() throws IOException, InterruptedException {
      ...
      synchronized (AdlsTransactionLog.this) {
        if (trace) {
    ...
    public String toString() {
      synchronized (AdlsTransactionLog.this) {
        return "AdlsLogReader{" + "file=" + tlogFile + ", position=" + fis.position() + ", end=" + getLogSize() + "}";
      }
    }
  ...
  public class AdlsSortedLogReader extends AdlsLogReader{
    private long startingPos;
    private boolean inOrder = true;
    private TreeMap<Long, Long> versionToPos;
    Iterator<Long> iterator;

    public AdlsSortedLogReader(long startingPos) {
      super(startingPos);
  ...
  public class AdlsReverseReader extends ReverseReader {
    AdlsDataFastInputStream fis;
    private LogCodec codec = new LogCodec(resolver) {
      @Override
    ...
    public AdlsReverseReader() throws IOException {
      incref();
      ...
      fis = new AdlsDataFastInputStream(provider.getReadStream(tlogFile), 0);
...
class AdlsDataFastInputStream extends FastInputStream {
  private ADLFileInputStream  fis;

  public AdlsDataFastInputStream(ADLFileInputStream fis, long chPosition) {
