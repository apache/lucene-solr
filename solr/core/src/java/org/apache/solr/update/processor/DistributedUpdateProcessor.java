...
public abstract class DistributedUpdateProcessor extends UpdateRequestProcessor {
  ...
  protected boolean isCloneRequiredOnLeader() {
    return cloneRequiredOnLeader;
  }

  protected UpdateCommand getUpdateCommand() {
  ...
  protected SolrQueryResponse getRsp() {
    return rsp;
  }
  ...
  protected final boolean cloneRequiredOnLeader;
  ...
  public DistributedUpdateProcessor(SolrQueryRequest req,
    ...
    this.replicaType = parseReplicaType(cc);
    this.collection = parseCollection(cc);
...
//    }
  ...
  protected abstract String parseCollection(CoreContainer cc);

  protected abstract Replica.Type parseReplicaType(CoreContainer cc);
  ...
  /**
   * This method can be overridden to tamper with the cmd after the localAdd operation
  ...
  /**
   * This method can be overridden to tamper with the cmd after the localDeleteByQuery operation
   * @param cmd the delete command
   * @throws IOException in case post processing failed
   */
  protected void postProcessDeleteByQuery(DeleteUpdateCommand cmd) throws IOException {
    // no-op for derived classes to implement
  }
  ...
  protected boolean versionAdd(AddUpdateCommand cmd) throws IOException {
    ...
    try {
      synchronized (bucket) {
        ...
        SolrInputDocument clonedDoc = cloneCmdDoc(cmd);
   ...
   *
   * @param cmd the update command
   * @return null if clone is not required, or a solrInputDocument when it is required
   */
  ...
  public void doDeleteByQuery(DeleteUpdateCommand cmd) throws IOException {
    doDeleteByQuery(cmd, Collections.emptyList(), null);
    postProcessDeleteByQuery(cmd);
  }

  public void doDeleteByQuery(DeleteUpdateCommand cmd, List<Node> replicaNodes, DocCollection docCollection) throws IOException {
    ...
    postProcessDeleteByQuery(cmd, replicaNodes, docCollection);
   ...
   * @param replicaNodes list of Nodes that are replicas
   */
  protected void postProcessDeleteByQuery(DeleteUpdateCommand cmd, List<Node> replicaNodes, DocCollection docCollection) throws IOException {
    // no-op for derived classes to implement
  }
  ...
  protected void setVersionAndDoLocalCommit(CommitUpdateCommand cmd) throws IOException {
    if (vinfo != null) {
