...
class BlockJoinFieldFacetAccumulator {
  ...
  void migrateGlobal(){
    ...
    migrateGlobal(globalCounts, segmentAccums, ordinalMap.getGlobalOrds(currentSegment));
  }

  /** folds counts in segment ordinal space (segCounts) into global ordinal space (counts) 
   * copy paste-from {@link DocValuesFacets#migrateGlobal(int[], int[], LongValues)}*/
  void migrateGlobal(int counts[], long segCounts[], LongValues ordMap) {
