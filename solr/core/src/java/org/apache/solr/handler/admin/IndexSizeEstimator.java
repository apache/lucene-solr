...
public class IndexSizeEstimator {
  ...
  public void setSamplingPercent(float percent) throws IllegalArgumentException {
    ...
    if (reader.maxDoc() > samplingThreshold) {
      ...
      log.info("- number of documents {} larger than {}, sampling percent is {} and sampling step {}"
          , reader.maxDoc(), samplingThreshold, samplingPercent, samplingStep); //verified
      if (reader.maxDoc() / samplingStep < 10) {
        throw new IllegalArgumentException("Out of " + reader.maxDoc()
