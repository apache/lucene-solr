...
public class ZkController implements Closeable {
  ...
  public ZkController(final CoreContainer cc, String zkServerAddress, int zkClientConnectTimeout, CloudConfig cloudConfig, final Supplier<List<CoreDescriptor>> descriptorsSupplier)
    ...
    // Use the configured way to do cluster state change (Overseer queue vs distributed)
    distributedClusterChangeUpdater = new DistributedClusterChangeUpdater(cloudConfig.getDistributedClusterChangeUpdates());
  ...
  public void publish(final CoreDescriptor cd, final Replica.State state, boolean updateLastState, boolean forcePublish) throws Exception {
    ...
    try {
      ...
      if (forcePublish || updateStateDotJson(coll, coreNodeName)) {
        if (distributedClusterChangeUpdater.isDistributedStateChange()) {
          distributedClusterChangeUpdater.doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.ReplicaSetState, m,
  ...
  public ZkDistributedQueue getOverseerJobQueue() {
    if (distributedClusterChangeUpdater.isDistributedStateChange()) {
      throw new IllegalStateException("Cluster is configured with distributed state change, not expecting the queue to be retrieved");
    }
