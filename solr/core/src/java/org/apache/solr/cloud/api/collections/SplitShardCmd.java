    ...
    try {
      ...
      for (int i = 0; i < subRanges.size(); i++) {
        ...
        if (ocmh.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
          ocmh.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.CollectionCreateShard, new ZkNodeProps(propMap),
      ...
      if (hasRecordedDistributedUpdate && ocmh.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
      ...
      if (leaderZnodeStat == null || ephemeralOwner != leaderZnodeStat.getEphemeralOwner()) {
        ...
        ZkNodeProps m = new ZkNodeProps(propMap);
        if (ocmh.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
          ocmh.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.SliceUpdateShardState, m,
              ocmh.cloudManager, ocmh.zkStateReader);
        } else {
