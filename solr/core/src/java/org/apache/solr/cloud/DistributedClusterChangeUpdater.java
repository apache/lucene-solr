...
/**
 * Gives access to distributed cluster state update methods and allows code to inquire whether distributed state change is enabled.
 */
public class DistributedClusterChangeUpdater {
   ...
   */
  private final boolean useDistributedStateChange;

  /**
   * Builds an instance with the specified behavior regarding distribution of state updates, allowing to know distributed
   * updates are not enabled (parameter {@code useDistributedStateChange} is {@code false}), or when they are (parameter
   * is {@code true)}, gives access to methods and classes allowing the execution of the updates.
   *
   * @param useDistributedStateChange when this parameter is {@code false}, only method expected to ever be called on this
   *                                  instance is {@link #isDistributedStateChange}, and it will return {@code false}.
   */
  public DistributedClusterChangeUpdater(boolean useDistributedStateChange) {
    this.useDistributedStateChange = useDistributedStateChange;
    if (log.isInfoEnabled()) {
      log.info("Creating DistributedClusterChangeUpdater with useDistributedStateChange=" + useDistributedStateChange
      + ". Solr will be using " + (useDistributedStateChange ? "distributed" : "Overseer based") + " cluster state updates."); // nowarn
    }
  }
  ...
  public StateChangeRecorder createStateChangeRecorder(String collectionName, boolean isCollectionCreation) {
    if (!useDistributedStateChange) {
      // Seeing this exception or any other of this kind here means there's a big bug in the code. No user input can cause this.
      throw new IllegalStateException("Not expecting to create instances of StateChangeRecorder when not using distributed state change");
    }
  ...
  public void doSingleStateUpdate(MutatingCommand command, ZkNodeProps message,
    ...
    if (!useDistributedStateChange) {
      throw new IllegalStateException("Not expecting to execute doSingleStateUpdate when not using distributed state change");
    }
  ...
  public void executeNodeDownStateChange(String nodeName, ZkStateReader zkStateReader) {
    if (!useDistributedStateChange) {
      throw new IllegalStateException("Not expecting to execute executeNodeDownStateChange when not using distributed state change");
    }
    CollectionNodeDownChangeCalculator.executeNodeDownStateChange(nodeName, zkStateReader);
  }

  /**
   * When this method returns {@code false} the legacy behavior of enqueueing cluster state change messages to Overseer
   * should be used and no other method of this class should be called.
   */
  public boolean isDistributedStateChange() {
    return useDistributedStateChange;
  }
   ...
   * it is handled specifically in {@link CollectionNodeDownChangeCalculator#executeNodeDownStateChange}.
   */
  public enum MutatingCommand {
     ...
     * {@code null} if no cluster state change command is defined for that action (given that {@link org.apache.solr.common.params.CollectionParams.CollectionAction}
  ...
  static private class ZkUpdateApplicator {
    ...
    private void applyUpdate() throws KeeperException, InterruptedException {
      ...
      for (int attempt = 0; attempt < CAS_MAX_ATTEMPTS; attempt++) {
        ...
        // TODO Eventually rethink the way each node manages and caches its cached image of the cluster state. Knowing about all collections in the cluster might not be needed.
     ...
     * as a special case. (see comment on {@link DistributedClusterChangeUpdater.StateChangeRecorder.RecordedMutationsPlayer}
  ...
  public static class StateChangeRecorder {
     ...
     * the distribution of the cluster state change updates.
     */
    ...
    public void executeStateUpdates(SolrCloudManager scm, ZkStateReader zkStateReader) throws KeeperException, InterruptedException {
      ...
      // This would replace the stats about cluster state changes that the Collection API currently makes available using
   ...
   *
   * Instances of this class deal with a single collection. Static method {@link #executeNodeDownStateChange} is the entry point
  ...
  private static class CollectionNodeDownChangeCalculator implements StateChangeCalculator {
     ...
     */
    public static void executeNodeDownStateChange(String nodeName, ZkStateReader zkStateReader) {
