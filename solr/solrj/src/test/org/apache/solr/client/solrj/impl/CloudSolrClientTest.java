...
public class CloudSolrClientTest extends SolrCloudTestCase {
  ...
  @Test
  public void testAddsResponse() throws Exception {

    CollectionAdminRequest.createCollection("addscol", "conf", 1, 1)
        .processAndWait(cluster.getSolrClient(), TIMEOUT);
    AbstractDistribZkTestBase.waitForRecoveriesToFinish("addscol", cluster.getSolrClient().getZkStateReader(), false, true, TIMEOUT);

    UpdateRequest updateRequest = new UpdateRequest()
        .add("id", "1", "a_t", "hello1")
        .add("id", "2", "a_t", "hello2");
    updateRequest.setParam(UpdateParams.VERSIONS, Boolean.TRUE.toString());
    UpdateResponse updateResponse = updateRequest.commit(cluster.getSolrClient(), "addscol");
    ...
    QueryResponse resp = cluster.getSolrClient().query("addscol", new SolrQuery("*:*"));
