 ...
 */
public class FastInputStreamSemiThreadSafe extends DataInputInputStream {
  ...
  public FastInputStreamSemiThreadSafe(InputStream in) {
  // use default BUFSIZE of BufferedOutputStream so if we wrap that
  // it won't cause double buffering.
    this(in, new byte[8192], 0, 0);
  }

  public FastInputStreamSemiThreadSafe(InputStream in, byte[] tempBuffer, int start, int end) {
    this.in = in;
    this.buf = tempBuffer;
    this.pos = start;
    this.end = end;
  ...
  public static FastInputStreamSemiThreadSafe wrap(InputStream in) {
    return (in instanceof FastInputStreamSemiThreadSafe) ? (FastInputStreamSemiThreadSafe)in : new FastInputStreamSemiThreadSafe(in);
  }
