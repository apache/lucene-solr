 ...
 */
public class FastOutputStreamSemiThreadSafe extends OutputStream implements DataOutput {
  protected final OutputStream out;
  protected byte[] buf;
  protected long written;  // how many bytes written to the underlying stream
  protected int pos;

  public FastOutputStreamSemiThreadSafe(OutputStream w) {
  // use default BUFSIZE of BufferedOutputStream so if we wrap that
  // it won't cause double buffering.
    this(w, new byte[8192], 0);
  }

  public FastOutputStreamSemiThreadSafe(OutputStream sink, byte[] tempBuffer, int start) {
    this.out = sink;
    this.buf = tempBuffer;
    this.pos = start;
  }
  ...
  public static FastOutputStreamSemiThreadSafe wrap(OutputStream sink) {
   return (sink instanceof FastOutputStreamSemiThreadSafe) ? (FastOutputStreamSemiThreadSafe)sink : new FastOutputStreamSemiThreadSafe(sink);
  }
