...
otherwise you open yourself up to attacks.   However, in development, it may be useful to use regular http urls, and bypass the
security check that Solr makes. To support this you can set the environment variable `solr.auth.jwt.allowOutboundHttp=true`. 
