...
The plugin will block anonymous traffic in this mode, since the default for `blockUnknown` is true. It is then possible to start configuring the plugin using REST API calls, which is described below.
...
blockUnknown         ; Set to `true` in order to block requests from users without a token  ; `truee`
