...
To index the example docs into a `gettingstarted` collection, run the following command:
...
The `-Dauto` parameter sends the appropriate document type to the appropriate extraction endpoints. 