...
Replication:: Replicate indexes for SolrCloud recovery and Primary/Secondary index distribution. This handler must have a core name in the path to the endpoint.
+
[cols="3*.",frame=none,grid=cols,options="header"]
|===
|API Endpoint |Class & Javadocs |Paramset
