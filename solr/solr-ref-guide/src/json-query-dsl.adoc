...
This paragraph binds many features together. Basically it's an example of filter exclusions for nested documents when facets are counted over children document (referred as SKUs in example below) fields, but facet counts rolls up in parent document counts. Let's go on item by item:
 ...
 - _nested documents_ or children documents are described in <<other-parsers.adoc#block-join-query-parsers>>
...
----
<1> define parents' filter via <<Additional Queries,additional queries>> for later usages
<2> declares two SKU filters under different #tags <<Tagging in JSON Query DSL>>
<3> tag block join query for later exclusion
<4> block join query joins SKUs to top level docs. See <<other-parsers.adoc#block-join-parent-query-parser>>
<5> referring to parent documents query. See <<Additional Queries,additional queries>>
