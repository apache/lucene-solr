...
This parameter sets the maximum number of searchers that may be warming up in the background at any given time. Exceeding this limit will raise an error. For read-only secondaries, a value of two is reasonable. Primary servers should probably be set a little higher.
