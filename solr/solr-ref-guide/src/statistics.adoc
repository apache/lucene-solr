...
In the example below a histogram is used to visualize a random sample of response times
...
The example below uses the probability distribution sampling capability
described in the link:probability.html[Probability] section of the user guide. The same
technique can be with random samples taken using the `random` function on empirical data
stored in Solr Cloud collections. But its also very useful to be able to
...
Then the percentile function is then used to calculate the percentiles for
both distributions.
...
as because the percentile values ar less tighly clustered around the mean.
