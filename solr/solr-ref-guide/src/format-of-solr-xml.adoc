...
`allowPaths`::
Cores can by default only be created relative to `$SOLR_HOME`, `$SOLR_DATA_HOME` or `coreRootDir`. If you need to create a core outside of these paths, you can explicitly allow the path with `allowPaths`. It is a comma separated string of file system paths to allow.
