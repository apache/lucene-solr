...
* `BinaryDocValuesStringField` ^<<fpbuc_1,1>>^:
...
1. [[fpbuc_1,1]] `BinaryDocValuesStringField` is intended for retrieval of large string values (e.g., for `/export` handler, streaming expressions, or `fl` param). Use for faceting and sorting is not supported.
