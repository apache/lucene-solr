...
<queryResponseWriter name="xslt"
                     class="org.apache.solr.scripting.xslt.XSLTResponseWriter">
...
[source,bash]
----
curl -o docs_to_add.xml "http://localhost:8983/solr/techproducts/select?q=ipod&fl=id,cat,name,popularity,price,score&wt=xslt&tr=updateXml.xsl"
curl -X POST -H "Content-Type: application/xml" -d @docs_to_add.xml 'http://localhost:8983/solr/techproducts/update?commitWithin=1000&overwrite=true"
