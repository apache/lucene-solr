...
of the load of query requests. This is used only if work stealing is enabled and acts
as the threshold beyond which query rate limiter will not allow other request types to
steal slots from its quota. Default value is allowed number of concurrent requests / 2.
[source,xml]
----
