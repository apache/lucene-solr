...
on the *x* axis and *miles* on *y* axis, and the *derivative* as *mph*,
at each *x* axis point.

Notice that the *miles_traveled* line has a slope of 10 until the
5th hour where
it changes to a slope of 50. The *mph* line create by
...
The second derivative is plotted as *acceleration* on the chart. Notice
that the second derivative shows the rate of change of firs
