...
Notice that the plot *visualizes* the volume under the curve as the *amzn* stock
price changes over time.  Because this plot is cumulative, the volume under
a stock price time line which stays the same over time will
have a *linear* slope. A stock that has rising prices will have a *concave* shape and
