...
[[modifyalias]]
== MODIFYALIAS: Modify Alias Properties for a Collection

The `MODIFYALIAS` action modifies the properties (metadata) on an alias. If a key is set with a value that is empty it will be removed.

`/admin/collections?action=MODIFYALIAS&name=_name_&properties.someKey=somevalue`
...
[source,text]
----
http://localhost:8983/solr/admin/collections?action=MODIFYALIAS&name=testalias2&properties.someKey=someValue&properties.otherKey=otherValue&wt=xml
----
