...
interval, then the next collection will be created asynchronously. Without this setting, collections are created
...
destined for a collection that does not exist will trigger synchronous creation up to that destination collection
but will not trigger additional async preemptive creation. Only one type of collection creation can happen
per document.
Example: `90MINUTES`.
+
This is blank by default indicating just-in-time, synchronous creation of new collections.
