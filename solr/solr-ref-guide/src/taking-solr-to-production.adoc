...
[NOTE]
====
Do not allocate a very large Java Heap memory unless you know you need it. Rule of thumb is to keep it as low as possible without running out of memory, since this will leave more free RAM for the Operating System to cache your index files automatically. Also, you should never go beyond 31Gb or heap since that will reduce performance and lead to more garbage collection, see <<#running-multiple-solr-nodes-per-host,Running Multiple Solr Nodes Per Host>>.
====
...
If you want to make this setting permanent, first make sure you have more than enough physical RAM on your host and then consult the documentation for correct procedure to disable swap.
...
Alternatively, you can change the setting temporarily, run this comamnd:
