...
To use the Analytics Component, the first step is to install this contrib module's plugins into Solr -- see the <<solr-plugins.adoc#installing-plugins,Solr Plugins>> section on how to do this. Note: method with `<lib>` directories doesn't work, consider copying `solr-analytic-x.x.jar` to `${solr.install.dir}/server/solr-webapp/webapp/WEB-INF/lib/`. 
