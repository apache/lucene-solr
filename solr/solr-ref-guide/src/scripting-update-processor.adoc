...
Note: There is a JavaScript example (update-script.js) as part of the `techproducts` configset and
an example collection1 configuration. Check `solrconfig.xml` and uncomment the
update request processor definition to enable this feature.
...
Put all JARs from a Groovy distro's `lib/` directory into Solr's resource loader
(core `lib/` directory, `<lib>` directive in `solrconfig.xml`, etc). All JARs from
...
Put the *standalone* `jython.jar` (the JAR that contains all the dependencies) into Solr's resource loader (core `lib/` directory, `<lib>` directive in `solrconfig.xml`, etc).
