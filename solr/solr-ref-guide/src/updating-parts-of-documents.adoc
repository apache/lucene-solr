...
`rejectSameVersion`::
This boolean parameter defaults to `true`, but if set to `false` the update will be rejected only if the value in the new document is less than the value in the existing document.
