...
For best results, the fields should have stored term vectors (`termVectors=true`), which can be <<defining-fields.adoc#defining-fields,configured in the schema>>.
...
|===
+
Query fields and their boosts using the same format used by the <<the-dismax-query-parser.adoc#the-dismax-query-parser,DisMax Query Parser>>.
...
This is accomplished with the use of <<content-streams.adoc#content-streams,Content Streams>>.
...
The structure of a MoreLikeThis query parser request is like a query using <<local-parameters-in-queries.adoc#local-parameters-in-queries,local params>>, as in:
