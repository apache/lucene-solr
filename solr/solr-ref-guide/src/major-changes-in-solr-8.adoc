...
* In the <<index-replication.adoc#index-replication,master-follower model>> of scaling Solr, a follower no longer commits an empty index when a completely new index is detected on master during replication. To return to the previous behavior pass `false` to `skipCommitOnMasterVersionZero` in the follower section of replication handler configuration, or pass it to the `fetchindex` command.
...
* In the ReplicationHandler, the `master.commitReserveDuration` sub-element is deprecated. Instead please configure a direct `commitReserveDuration` element for use in all modes (master, follower, cloud).
