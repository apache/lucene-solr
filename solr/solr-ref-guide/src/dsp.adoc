...
of the filter. Notice that the filter is reversed with `rev` function inside the `conv` function.
This is done because convolution reverses
the filter. In this case we reverse it ahead of time and convolution reverses it back, so it is the same
