...
[source,text]
----
let(a=random(testapp, q="*:*", rows="5000", fl="filesize_d, load_d, response_d"),
    b=col(a, filesize_d),
    c=col(a, load_d),
    d=col(a, response_d),
    e=transpose(matrix(b, c)),
...
Although we can't do a regression plot with multi-variate linear regression we can do a residual plot, and visualize the errors
of the regression. The residual plot for multi-variate linear regression is the same as for simple linear regression.
