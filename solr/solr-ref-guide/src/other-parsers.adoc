...
`PayloadCheckQParser` only matches when the matching terms also have the specified payloads.  The matching by default is an equals behavior,
however, inequality matching can also be performed to support greater than(gt), less than(lt), greater than or equal(gte), less than or equal(lte) when doing the payload matching.
...
`op`::
The inequality operation to apply to the payload check.  This will test for the payload on the term to be greater than / less than the payload value provided in the payloads parameter.  Valid values are: gt,gte,lt,lte,eq (default behavior is eq)
...
[source,text]
Find all documents with "foo bar" where term "foo" has a payload greater than 9 and "bar" has a payload greater than 5
