...
+
`docValuesTermsFilterPerSegment` uses segment-level doc values data structures to find results.  This is more general purpose than the corresponding "top level" method, but doesn't have the same advantages at large numbers of query terms (see above).
+
