...
** When more than one sort criteria is provided, the second entry will only be used if the first entry results in a tie. If there is a third entry, it will only be used if the first AND second entries are tied. And so on.  If documents tie in all of the specified sort criteria, Solr uses each document's Lucene document ID as the final tie-breaker.
