...
...
----
Since `hitCountExact=false`, we know the number of documents matching the query is greater or equal to 153. If we specify a higher value for `minExactHits`:

[source,text]
q=quick brown fox&minExactHits=200&rows=10
