...
It's important to be aware that many changes require reindexing, because failing to reindex can have both obvious and subtle consequences for Solr or for the ability of your users to find what they are looking for.

"Reindex" in this context means _first delete the existing index and repeat the process you used to ingest the entire corpus from the system-of-record_. It is strongly recommended that Solr users index their data in a repeatable, consistent way, so that the process can be easily repeated when the need for reindexing arises.

[CAUTION]
====
Re-ingesting all the documents in your corpus without first insuring that all documents and Lucene segments have been deleted is *not* sufficient, see the section below <<reindexing.adoc#reindexing-strategies,Reindexing Strategies>>.
====
...
With very few exceptions, changes to a collection's schema require reindexing. This is because many of the available options are only
applied during the indexing process. Solr simply has no way to implement the desired change without reindexing the data.

To understand the general reason why reindexing is ever required, it's helpful to understand the relationship between Solr's schema and the underlying Lucene index. Lucene does not use a schema, schemas a Solr-only concept. When you change Solr's schema, the Lucene's index is not modified in any way.
...
When you change your schema by adding fields, removing fields, or changing the field or field type definitions you generally do so with the intent use those changes in some way.
...
[CAUTION]
====
Changing any field properties without reindexing is _never_ recommended. This should only be attempted with a thorough understanding of the consequences. Negative impacts on the user may not be immediately apparent.

As an example, consider adding a new searchable field to the schema. Until all documents have been reindexed, any search against this new field can find a varying number of documents depending on how far along the reindex process has progressed.
...
Lucene works hard to insure one major version back-compatability, thus Solr 8x can read indexes created with Solr 7x. However, given that this guarantee does _not_ apply to Solr X-2 (Solr 6x in this example) we still recommend completely reindexing when moving from Solr X-1 to Solr X.
...
Therefore, since all documents have to be re-ingested to insure the abstraction faithfully reflects the new schema for all documents, we recommend deleting all documents or reindexing to a new collection.
====
...
With this approach, you will index your documents into a new collection that uses your changes and, once indexing and testing are complete, create an alias for the collection that points your front-end at the collection alias. Queries and updates will be routed to the new collection seamlessly.
...
[NOTE]
One advantage of this option is that if, for some reason, your testing did not uncover a problem you can switch back to the old collection until you fix the new one. Of course this option can require more resources until the old collection can be deleted.
