...
`out`::
The file name of the export. If the file name ends with `jsonl.gz` the output will be compressed into a .gz file.
...
Export all documents of collection `gettingstarted` into a file called `1MDocs.jsonl.gz` as a zipped JSON file:

[source,bash]
----
bin/solr export -url http://localhost:8983/solr/gettingstarted -1 -out 1MDocs.jsonl.gz
----
...
`curl -X POST -d @gettingstarted.jsonl http://localhost:8983/solr/gettingstarted/update/json/docs?commit=true`
