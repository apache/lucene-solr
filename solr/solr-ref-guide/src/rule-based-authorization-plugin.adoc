...
=== Example for RuleBasedAuthorizationPlugin
...
"authentication":{
   ...
   "jwk_url": "https://my.identity.provider/my.wks", <2>
   "roles_claim": "groups" <3>
},
"authorization":{
...
<2> Public key to Identity server is pulled over https
<3> We expect each JWT token to contain a "groups" claim, which will be passed on to Authorization
