...
/**
 * Suggester based on a weighted FST: it first traverses the prefix, then walks the <i>n</i>
 * shortest paths to retrieve top-ranked suggestions.
 *
 * <p><b>NOTE</b>: Input weights must be between 0 and {@link Integer#MAX_VALUE}, any other values
 * will be rejected.
 *
 * @lucene.experimental
 */
...
public class WFSTCompletionLookup extends Lookup implements Accountable {
  ...
  /**
   * Calls {@link #WFSTCompletionLookup(Directory,String,boolean)
   ...
   *
   * @param exactFirst <code>true</code> if suggestions that match the prefix exactly should always
   *     be returned first, regardless of score. This has no performance impact, but could result in
  ...
  private static final class WFSTInputIterator extends SortedInputIterator {
    ...
    WFSTInputIterator(Directory tempDir, String tempFileNamePrefix, InputIterator source)
        throws IOException {
