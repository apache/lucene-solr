...
public class AnalyzingInfixSuggesterTest extends LuceneTestCase {
  ...
  private void testConstructorDefaults(
    ...
    AnalyzingInfixSuggester suggester2 =
        new AnalyzingInfixSuggester(newDirectory(), a, a, 3, false, allTermsRequired, highlight);
  ...
  public void testHighlightAsObject() throws Exception {
    ...
    Analyzer a = new MockAnalyzer(random(), MockTokenizer.WHITESPACE, false);
    AnalyzingInfixSuggester suggester =
        new AnalyzingInfixSuggester(newDirectory(), a, a, 3, false) {
          @Override
          protected Object highlight(String text, Set<String> matchedTokens, String prefixToken)
            ...
            try (TokenStream ts = queryAnalyzer.tokenStream("text", new StringReader(text))) {
              ...
              while (ts.incrementToken()) {
                ...
                if (upto < startOffset) {
                  fragments.add(
                      new LookupHighlightFragment(text.substring(upto, startOffset), false));
                  upto = startOffset;
                ...
                if (matchedTokens.contains(token)) {
                  // Token matches.
                  fragments.add(
                      new LookupHighlightFragment(text.substring(startOffset, endOffset), true));
                  upto = endOffset;
                } else if (prefixToken != null && token.startsWith(prefixToken)) {
                  fragments.add(
                      new LookupHighlightFragment(
                          text.substring(startOffset, startOffset + prefixToken.length()), true));
    ...
    assertEquals(1, results.size());
    assertEquals(
        "a penny saved is a penny <b>ear</b>ned",
        toString((List<LookupHighlightFragment>) results.get(0).highlightKey));
    assertEquals(10, results.get(0).value);
    assertEquals(new BytesRef("foobaz"), results.get(0).payload);
    suggester.close();
    a.close();
  ...
  public void testRandomMinPrefixLength() throws Exception {
    ...
    AnalyzingInfixSuggester suggester =
        new AnalyzingInfixSuggester(newFSDirectory(tempDir), a, a, minPrefixLength, false);
    suggester.build(new InputArrayIterator(keys));
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        List<LookupResult> results =
            suggester.lookup(TestUtil.stringToCharSequence("ear", random()), 10, true, doHighlight);
        assertEquals(2, results.size());
        assertEquals("a penny saved is a penny earned", results.get(0).key);
        if (doHighlight) {
          assertEquals("a penny saved is a penny <b>ear</b>ned", results.get(0).highlightKey);
        ...
        results =
            suggester.lookup(
                TestUtil.stringToCharSequence("ear ", random()), 10, true, doHighlight);
        assertEquals(1, results.size());
        assertEquals("lend me your ear", results.get(0).key);
        if (doHighlight) {
          assertEquals("lend me your <b>ear</b>", results.get(0).highlightKey);
        ...
        results =
            suggester.lookup(TestUtil.stringToCharSequence("pen", random()), 10, true, doHighlight);
        assertEquals(1, results.size());
        assertEquals("a penny saved is a penny earned", results.get(0).key);
        if (doHighlight) {
          assertEquals(
              "a <b>pen</b>ny saved is a <b>pen</b>ny earned", results.get(0).highlightKey);
        }
        assertEquals(10, results.get(0).value);
        assertEquals(new BytesRef("foobaz"), results.get(0).payload);

        results =
            suggester.lookup(TestUtil.stringToCharSequence("p", random()), 10, true, doHighlight);
        assertEquals(1, results.size());
        assertEquals("a penny saved is a penny earned", results.get(0).key);
        if (doHighlight) {
          assertEquals(
              "a <b>p</b>enny saved is a <b>p</b>enny earned", results.get(0).highlightKey);
        }
        assertEquals(10, results.get(0).value);
        assertEquals(new BytesRef("foobaz"), results.get(0).payload);
      }
  ...
  public void testHighlight() throws Exception {
    ...
    suggester.build(new InputArrayIterator(keys));
    List<LookupResult> results =
        suggester.lookup(TestUtil.stringToCharSequence("penn", random()), 10, true, true);
    assertEquals(1, results.size());
    assertEquals("a penny saved is a penny earned", results.get(0).key);
  ...
  public void testSuggestStopFilter() throws Exception {
    ...
    suggester.build(new InputArrayIterator(keys));
    List<LookupResult> results =
        suggester.lookup(TestUtil.stringToCharSequence("a", random()), 10, true, true);
    assertEquals(1, results.size());
  ...
  public void testEmptyAtStart() throws Exception {
    ...
    suggester.add(
        new BytesRef("a penny saved is a penny earned"), null, 10, new BytesRef("foobaz"));
  ...
  public void testBothExactAndPrefix() throws Exception {
    ...
    List<LookupResult> results =
        suggester.lookup(TestUtil.stringToCharSequence("pen p", random()), 10, true, true);
    assertEquals(1, results.size());
  ...
  public void testBinaryContext() throws Exception {
    ...
    for (int iter = 0; iter < 2; iter++) {
      ...
      List<LookupResult> results =
          suggester.lookup(
              TestUtil.stringToCharSequence("ear", random()), asSet(context1), 10, true, true);
      assertEquals(2, results.size());
  ...
  public void testContextNotAllTermsRequired() throws Exception {
    ...
    // No context provided, all results returned
    List<LookupResult> results =
        suggester.lookup(TestUtil.stringToCharSequence("ear", random()), 10, false, true);
    assertEquals(2, results.size());
    LookupResult result = results.get(0);
    assertEquals("a penny saved is a penny earned", result.key);
    assertEquals("a penny saved is a penny <b>ear</b>ned", result.highlightKey);
    ...
    results =
        suggester.lookup(
            TestUtil.stringToCharSequence("ear", random()), asSet("foo"), 10, false, true);
    assertEquals(2, results.size());
    ...
    results =
        suggester.lookup(
            TestUtil.stringToCharSequence("len", random()), asSet("foo"), 10, false, true);
    assertEquals(1, results.size());
  ...
  public void testCloseIndexWriterOnBuild() throws Exception {
    class MyAnalyzingInfixSuggester extends AnalyzingInfixSuggester {
      public MyAnalyzingInfixSuggester(
          Directory dir,
          Analyzer indexAnalyzer,
          Analyzer queryAnalyzer,
          int minPrefixChars,
    ...
    Analyzer a = new MockAnalyzer(random(), MockTokenizer.WHITESPACE, false);
    Path tempDir = createTempDir("analyzingInfixContext");
    final MyAnalyzingInfixSuggester suggester =
        new MyAnalyzingInfixSuggester(
    ...
    expectThrows(
        AlreadyClosedException.class, () -> suggester.getSearcherManager().maybeRefreshBlocking());
    ...
    // * The IndexWriter should be null
    // * The SearcherManager should be non-null
    final MyAnalyzingInfixSuggester suggester2 =
        new MyAnalyzingInfixSuggester(
            newFSDirectory(tempDir),
            a,
            a,
