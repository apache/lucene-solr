...
public class TestSegmentTermDocs extends LuceneTestCase {
  ...
  public void testSkipTo() throws IOException {
    ...
    PostingsEnum tdocs =
        TestUtil.docs(
            random(), reader, ta.field(), new BytesRef(ta.text()), null, PostingsEnum.FREQS);
    ...
    // without next
    tdocs = TestUtil.docs(random(), reader, ta.field(), new BytesRef(ta.text()), null, 0);
    ...
    tdocs =
        TestUtil.docs(
            random(), reader, tb.field(), new BytesRef(tb.text()), null, PostingsEnum.FREQS);
    ...
    tdocs =
        TestUtil.docs(
            random(), reader, tc.field(), new BytesRef(tc.text()), null, PostingsEnum.FREQS);
    ...
    // without next
    tdocs = TestUtil.docs(random(), reader, tc.field(), new BytesRef(tc.text()), null, 0);
    assertTrue(tdocs.advance(5) != DocIdSetIterator.NO_MORE_DOCS);
    assertEquals(26, tdocs.docID());
    assertTrue(tdocs.advance(40) != DocIdSetIterator.NO_MORE_DOCS);
    assertEquals(40, tdocs.docID());
