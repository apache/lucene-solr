...
public class TestFieldMaskingSpanQuery extends LuceneTestCase {
  ...
  public void testNoop1() throws Exception {
    ...
    q =
        new SpanNearQuery(
            new SpanQuery[] {
              new FieldMaskingSpanQuery(q1, "last"), new FieldMaskingSpanQuery(q2, "last")
            },
            0,
            true);
    check(q, new int[] {1, 2});
  ...
  public void testSimple2() throws Exception {
    ...
    q =
        new SpanNearQuery(
            new SpanQuery[] {
              new FieldMaskingSpanQuery(q1, "id"), new FieldMaskingSpanQuery(q2, "id")
            },
            -1,
            false);
    check(q, new int[] {2, 4});
  ...
  public void testSpans1() throws Exception {
    ...
    Spans spanA =
        qA.createWeight(searcher, ScoreMode.COMPLETE_NO_SCORES, 1f)
            .getSpans(searcher.getIndexReader().leaves().get(0), SpanWeight.Postings.POSITIONS);
    Spans spanB =
        qB.createWeight(searcher, ScoreMode.COMPLETE_NO_SCORES, 1f)
            .getSpans(searcher.getIndexReader().leaves().get(0), SpanWeight.Postings.POSITIONS);
  ...
  public void testSpans2() throws Exception {
    ...
    SpanQuery q =
        new SpanNearQuery(
            new SpanQuery[] {
              new FieldMaskingSpanQuery(qA, "id"), new FieldMaskingSpanQuery(qB, "id")
            },
            -1,
            false);
