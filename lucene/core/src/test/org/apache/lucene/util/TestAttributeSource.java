...
public class TestAttributeSource extends LuceneTestCase {
  ...
  public void testCloneAttributes() {
    ...
    assertNotSame(
        "FlagsAttribute of original and clone must be different instances", flagsAtt2, flagsAtt);
    assertNotSame(
        "TypeAttribute of original and clone must be different instances", typeAtt2, typeAtt);
    ...
    clone.copyTo(src);
    assertEquals(
        "FlagsAttribute of original must now contain updated term", 4711, flagsAtt.getFlags());
    assertEquals(
        "TypeAttribute of original must now contain updated type", "OtherType", typeAtt.type());
    // verify again:
    assertNotSame(
        "FlagsAttribute of original and clone must be different instances", flagsAtt2, flagsAtt);
    assertNotSame(
  ...
  public void testDefaultAttributeFactory() throws Exception {
    ...
    assertTrue(
        ...
        src.addAttribute(PositionIncrementAttribute.class)
            instanceof PositionIncrementAttributeImpl);
    assertTrue(
  ...
  public void testRemoveAllAttributes() {
    ...
    for (Class<? extends Attribute> attrClass : attrClasses) {
      ...
      assertTrue(
          "Missing added attribute " + attrClass.getSimpleName(),
    ...
    for (Class<? extends Attribute> attrClass : attrClasses) {
      assertFalse(
          "Didn't remove attribute " + attrClass.getSimpleName(),
          defaultFactoryAttributeSource.hasAttribute(attrClass));
    }
    assertFalse(defaultFactoryAttributeSource.hasAttributes());
