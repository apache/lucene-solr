...
public class Test2BBKDPoints extends LuceneTestCase {
  public void test1D() throws Exception {
    ...
    BKDConfig config = new BKDConfig(1, 1, Long.BYTES, BKDConfig.DEFAULT_MAX_POINTS_IN_LEAF_NODE);
    BKDWriter w = new BKDWriter(config, numDocs, dir, "_0", BKDWriter.DEFAULT_MAX_MB_SORT_IN_HEAP, 26L * numDocs);
    int counter = 0;
    ...
    long indexFP = w.finish(new BKDDefaultIndexWriter(out));
    out.close();
  ...
  public void test2D() throws Exception {
    ...
    final int numDocs = (Integer.MAX_VALUE / 26) + 100;
    BKDConfig config = new BKDConfig(2, 2, Long.BYTES, BKDConfig.DEFAULT_MAX_POINTS_IN_LEAF_NODE);
    BKDWriter w = new BKDWriter(config, numDocs, dir, "_0", BKDWriter.DEFAULT_MAX_MB_SORT_IN_HEAP, 26L * numDocs);
    int counter = 0;
