...
public class TestPolygon2D extends LuceneTestCase {
  ...
  /**
   * If polygon.intersects(box) returns false, then any point in that box should return false as
   * well
   */
   ...
   * well
   */
  // different from testIntersectsRandom in that its not a purely random test. we iterate the
  ...
  public void testEdgeInsideness() {
    Component2D poly =
        Polygon2D.create(
            new Polygon(new double[] {-2, -2, 2, 2, -2}, new double[] {-2, 2, 2, -2, -2}));
