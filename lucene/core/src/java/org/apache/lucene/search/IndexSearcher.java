 ...
 * method. For performance reasons, if your index is unchanging, you should
 * share a single IndexSearcher instance across multiple searches instead of
 * creating a new one per-search. If your index has changed and you wish to see
 * the changes reflected in searching, you should use
 * {@link DirectoryReader#openIfChanged(DirectoryReader)} to obtain a new reader
 * and then create a new IndexSearcher from that. Also, for low-latency
 * turnaround it's best to use a near-real-time reader
 * ({@link DirectoryReader#open(IndexWriter)}). Once you have a new
 * {@link IndexReader}, it's relatively cheap to create a new IndexSearcher from
 * it.
 *
 * <p>
 * <b>NOTE</b>: The {@link #search} and {@link #searchAfter} methods are
 * configured to only count top hits accurately up to {@code 1,000} and may
 * return a {@link TotalHits.Relation lower bound} of the hit count if the hit
 * count is greater than or equal to {@code 1,000}. On queries that match lots
 * of documents, counting the number of hits may take much longer than computing
 * the top hits so this trade-off allows to get some minimal information about
 * the hit count without slowing down search too much. The
 ...
 * <p>
 * <b>NOTE</b>: <code>{@link
 * IndexSearcher}</code> instances are completely thread safe, meaning multiple
 * threads can call any of its methods, concurrently. If your application
 * requires external synchronization, you should <b>not</b> synchronize on the
 * <code>IndexSearcher</code> instance; use your own (non-Lucene) objects
 * instead.
 */
public class IndexSearcher {
  ...
  static {
    ...
    final long maxRamBytesUsed = Math.min(1L << 25,
  ...
  /**
   * Expert: returns a default Similarity instance. In general, this method is
   * only called to initialize searchers and writers. User code and query
   * implementations should respect {@link IndexSearcher#getSimilarity()}.
   *
   * @lucene.internal
   */
   ...
   * @lucene.internal
   */
  public static void setDefaultQueryCachingPolicy(
   ...
   * NOTE: if you are using {@link NIOFSDirectory}, do not use the shutdownNow
   * method of ExecutorService as this uses Thread.interrupt under-the-hood
   ...
   * {@link IndexReaderContext}.
   *
   * <p>
   * Given a non-<code>null</code> {@link Executor} this method runs searches
   * for each segment separately, using the provided Executor. NOTE: if you are
   * using {@link NIOFSDirectory}, do not use the shutdownNow method of
   * ExecutorService as this uses Thread.interrupt under-the-hood which can
   * silently close file descriptors (see <a href=
   * "https://issues.apache.org/jira/browse/LUCENE-2239">LUCENE-2239</a>).
   *
  ...
  /**
   * Set the maximum number of clauses permitted per Query. Default value is
   * 1024.
   */
  ...
  /**
   * Set the {@link QueryCache} to use when scores are not needed. A value of
   * {@code null} indicates that query matches should never be cached. This
   * method should be called <b>before</b> starting using this
   * {@link IndexSearcher}.
   *
   * <p>
   * NOTE: When using a query cache, queries should not be modified after they
  ...
  /**
   * Set the {@link QueryCachingPolicy} to use for query caching. This method
   * should be called <b>before</b> starting using this {@link IndexSearcher}.
   *
   ...
   * that was last set through
   * {@link #setQueryCachingPolicy(QueryCachingPolicy)}.
   *
   * @lucene.experimental
   */
   ...
   * segments with more than MAX_DOCS_PER_SLICE will get their own thread
   */
  ...
  public static LeafSlice[] slices(List<LeafReaderContext> leaves,
      int maxDocsPerSlice, int maxSegmentsPerSlice) {
    // Make a copy so we can sort:
    List<LeafReaderContext> sortedLeaves = new ArrayList<>(leaves);
    
    // Sort by maxDoc, descending:
    Collections.sort(sortedLeaves, Collections
        .reverseOrder(Comparator.comparingInt(l -> l.reader().maxDoc())));
  ...
  /**
   * Expert: Get the {@link Similarity} to use to compute scores. This returns
   * the {@link Similarity} that has been set through
   * {@link #setSimilarity(Similarity)} or the default {@link Similarity} if
   ...
   * {@code Executor} was passed to the constructor.
   *
   * @lucene.experimental
   */
  ...
  /**
   * Finds the top <code>n</code> hits for <code>query</code> where all results
   * are after a previous result (<code>after</code>).
   *
   * <p>
   ...
   *           If a query would exceed {@link IndexSearcher#getMaxClauseCount()}
   *           clauses.
   */
  public TopDocs searchAfter(ScoreDoc after, Query query, int numHits)
    ...
    final CollectorManager<TopScoreDocCollector,TopDocs> manager = new CollectorManager<TopScoreDocCollector,TopDocs>() {
      ...
      private final HitsThresholdChecker hitsThresholdChecker = (executor == null
          || leafSlices.length <= 1)
              ? HitsThresholdChecker
                  .create(Math.max(TOTAL_HITS_THRESHOLD, numHits))
              : HitsThresholdChecker
                  .createShared(Math.max(TOTAL_HITS_THRESHOLD, numHits));
      
      private final MaxScoreAccumulator minScoreAcc = (executor == null
          || leafSlices.length <= 1) ? null : new MaxScoreAccumulator();
      
      @Override
      public TopScoreDocCollector newCollector() throws IOException {
        return TopScoreDocCollector.create(cappedNumHits, after,
            hitsThresholdChecker, minScoreAcc);
      }
  ...
  /**
   * Search implementation with arbitrary sorting, plus control over whether hit
   * scores and max score should be computed. Finds the top <code>n</code> hits
   * for <code>query</code>, and sorting the hits by the criteria in
   * <code>sort</code>. If <code>doDocScores</code> is <code>true</code> then
   * the score of each hit will be computed and returned. If
   * <code>doMaxScore</code> is <code>
   * true</code> then the maximum score over all collected hits will be
   * computed.
   *
   * @throws TooManyClauses
   *           If a query would exceed {@link IndexSearcher#getMaxClauseCount()}
   ...
   * @param sort
   *          The {@link org.apache.lucene.search.Sort} object
   * @return The top docs, sorted according to the supplied
   *         {@link org.apache.lucene.search.Sort} instance
   * @throws IOException
  ...
  /**
   * Finds the top <code>n</code> hits for <code>query</code> where all results
   * are after a previous result (<code>after</code>), allowing control over
   * whether hit scores and max score should be computed.
   *
   * <p>
   * By passing the bottom result from a previous page as <code>after</code>,
   * this method can be used for efficient 'deep-paging' across potentially
   * large result sets. If <code>doDocScores
   * </code> is <code>true</code> then the score of each hit will be computed
   * and returned. If <code>doMaxScore</code> is <code>true</code> then the
   * maximum score over all collected hits will be computed.
   *
   * @throws TooManyClauses
   *           If a query would exceed {@link IndexSearcher#getMaxClauseCount()}
   *           clauses.
   */
  public TopFieldDocs searchAfter(ScoreDoc after, Query query, int numHits,
      Sort sort, boolean doDocScores) throws IOException {
    if (after != null && !(after instanceof FieldDoc)) {
      // TODO: if we fix type safety of TopFieldDocs we can
      // remove this
      throw new IllegalArgumentException(
          "after must be a FieldDoc; got " + after);
    }
    return searchAfter((FieldDoc) after, query, numHits, sort, doDocScores);
  }
  
  private TopFieldDocs searchAfter(FieldDoc after, Query query, int numHits,
      Sort sort, boolean doDocScores) throws IOException {
    final int limit = Math.max(1, reader.maxDoc());
    if (after != null && after.doc >= limit) {
      throw new IllegalArgumentException(
  ...
  /**
   * Lower-level search API. Search all leaves using the given
   * {@link CollectorManager}. In contrast to {@link #search(Query, Collector)},
   * this method will use the searcher's {@link Executor} in order to
   * parallelize execution of the collection on the configured
   * {@link #leafSlices}.
   *
  ...
  public <C extends Collector,T> T search(Query query,
      CollectorManager<C,T> collectorManager) throws IOException {
   ...
   *
   * <p>
   * NOTE: this method executes the searches on all given leaves exclusively. To
   * search across all the searchers leaves use {@link #leafContexts}.
   *
   * @param leaves
  ...
  /**
   * Returns a QueryVisitor which recursively checks the total number of clauses
   * that a query and its children cumulatively have and validates that the
   * total number does not exceed the specified limit
   */
  private static QueryVisitor getNumClausesCheckVisitor() {
    return new QueryVisitor() {
      ...
      @Override
      public QueryVisitor getSubVisitor(BooleanClause.Occur occur,
          Query parent) {
      ...
      @Override
      public void consumeTermsMatching(Query query, String field,
   ...
   *
   * <p>
   * This is intended to be used in developing Similarity implementations, and,
   * for good performance, should not be displayed with every hit. Computing an
  ...
  /**
   * Expert: low-level implementation method Returns an Explanation that
   * describes how <code>doc
  ...
  /**
   * Creates a {@link Weight} for the given query, potentially adding caching if
   ...
   *
   * <p>
   * This can be overridden for example, to return a term's statistics across a
   * distributed collection.
   *
   * @param docFreq
   *          The document frequency of the term. It must be greater or equal to
   *          1.
   * @param totalTermFreq
   *          The total term frequency.
   * @return A {@link TermStatistics} (never null).
   * @lucene.experimental
   */
  public TermStatistics termStatistics(Term term, int docFreq,
  ...
  /**
   * Returns {@link CollectionStatistics} for a field, or {@code null} if the
   * field does not exist (has no indexed terms)
   *
   * <p>
   * This can be overridden for example, to return a field's statistics across a
   * distributed collection.
   *
   * @lucene.experimental
   */
  public CollectionStatistics collectionStatistics(String field)
    ...
    return new CollectionStatistics(field, reader.maxDoc(), docCount,
        sumTotalTermFreq, sumDocFreq);
  }
  
  /**
   * Returns this searchers executor or <code>null</code> if no executor was
   * provided
   */
  ...
  /**
   * Return the SliceExecutionControlPlane instance to be used for this
   * IndexSearcher instance
   */
  private static SliceExecutor getSliceExecutionControlPlane(
      Executor executor) {
    if (executor == null) {
      return null;
    }
