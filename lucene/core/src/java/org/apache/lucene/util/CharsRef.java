...
public final class CharsRef implements Comparable<CharsRef>, CharSequence, Cloneable {
  ...
  /** Creates a new {@link CharsRef} initialized with an array of the given capacity */
  public CharsRef(int capacity) {
    chars = new char[capacity];
  }

  /** Creates a new {@link CharsRef} initialized with the given array, offset and length */
  ...
  /** Creates a new {@link CharsRef} initialized with the given Strings character array */
  ...
  /**
   * Returns a shallow clone of this instance (the underlying characters are <b>not</b> copied and
  ...
  /**
   * Creates a new CharsRef that points to a copy of the chars from <code>other</code>
   *
   * <p>The returned CharsRef will have a length of other.length and an offset of zero.
   */
  ...
  public boolean isValid() {
    ...
    if (length > chars.length) {
      throw new IllegalStateException(
          "length is out of bounds: " + length + ",chars.length=" + chars.length);
    }
    if (offset < 0) {
    ...
    if (offset > chars.length) {
      throw new IllegalStateException(
          "offset out of bounds: " + offset + ",chars.length=" + chars.length);
    }
    if (offset + length < 0) {
      throw new IllegalStateException(
