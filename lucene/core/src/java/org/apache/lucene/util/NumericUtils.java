...
public final class NumericUtils {
  ...
  /**
   * <p>Converts the specified single-precision float value into a sortable short by converting to
   * <a href="https://en.wikipedia.org/wiki/Half-precision_floating-point_format">IEEE 754</a>
   * 16-bit floating point format (reducing the float value's precision). Infinite values and NaN
   * are preserved, and subnormal (denorm) floats are handled.
   *
   * Derived from code published by Jeroen van der Zijp (see
   * http://www.fox-toolkit.org/ftp/fasthalffloatconversion.pdf) with permission from the author.
   */
  public final static class Float16Converter {

    private final short[] f16BaseTable;
    private final byte[] f16ShiftTable;

    public Float16Converter() {
      f16BaseTable = new short[512];
      f16ShiftTable = new byte[512];
      int i;
      int e;
      for(i=0; i<256; ++i) {
        e = i - 127;
        if (e < -24) {                  // Very small numbers map to zero
          f16BaseTable[i | 0x000] = 0x0000;
          f16BaseTable[i | 0x100] = (short) 0x8000;
          f16ShiftTable[i | 0x000] = 24;
          f16ShiftTable[i | 0x100] = 24;
        } else if (e < -14) {             // Small numbers map to denorms
          f16BaseTable[i | 0x000] = (short) ((0x0400 >> (-e - 14)));
          f16BaseTable[i | 0x100] = (short) ((0x0400 >> (-e - 14)) | 0x8000);
          f16ShiftTable[i | 0x000] = (byte) (-e - 1);
          f16ShiftTable[i | 0x100] = (byte) (-e - 1);
        } else if (e <= 15) {             // Normal numbers just lose precision
          f16BaseTable[i | 0x000] = (short) ((e + 15) << 10);
          f16BaseTable[i | 0x100] = (short) (((e + 15) << 10) | 0x8000);
          f16ShiftTable[i | 0x000] = 13;
          f16ShiftTable[i | 0x100] = 13;
        } else if (e < 128) {             // Large numbers map to Infinity
          f16BaseTable[i | 0x000] = 0x7C00;
          f16BaseTable[i | 0x100] = (short) 0xFC00;
          f16ShiftTable[i | 0x000] = 24;
          f16ShiftTable[i | 0x100] = 24;
    ...
    /* Returns a reduced precision (half-width) floating point value, as a short int
     * @param f The single-precision float value to convert to half-precision
     * @return A half-precision float value
     */
    public short floatToShortFloat(float f) {
      int bits = Float.floatToIntBits(f);
      return (short) (f16BaseTable[(bits >> 23) & 0x1ff] + ((bits & 0x007fffff) >> f16ShiftTable[(bits >> 23) & 0x1ff]));
    }

    /* Returns a sortable short value that sorts the same (up to loss of precision) as the original
     * float.
     * @param f The single-precision float value to convert to half-precision
     * @return A half-precision sortable float value
     */
    public short floatToSortableShort(float f) {
      int f16bits = floatToShortFloat(f);
      return (short) (f16bits ^ (f16bits >> 16) & 0x7fff);
    }
  }
