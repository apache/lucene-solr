...
public class BKDDefaultIndexWriter implements BKDIndexWriter {
  ...
  /** Packs the two arrays, representing a balanced binary tree, into a compact byte[] structure. */
  private byte[] packIndex(BKDConfig config, BKDTreeLeafNodes leafNodes) throws IOException {
    // This is the "file" we append the byte[] to:
    List<byte[]> blocks = new ArrayList<>();
    byte[] lastSplitValues = new byte[config.packedIndexBytesLength];
    //System.out.println("\npack index");
    assert scratchOut.size() == 0;
    int totalSize = recursePackIndex(scratchOut, config, leafNodes, 0l, blocks, lastSplitValues, new boolean[config.numIndexDims], false,
   ...
   */
  private int recursePackIndex(ByteBuffersDataOutput writeBuffer, BKDConfig config, BKDTreeLeafNodes leafNodes, long minBlockFP, List<byte[]> blocks,
    ...
    if (numLeaves == 1) {
      ...
      int leftNumBytes = recursePackIndex(writeBuffer, config, leafNodes, leftBlockFP, blocks, lastSplitValues, negativeDeltas, true,
      ...
      int rightNumBytes = recursePackIndex(writeBuffer, config, leafNodes, leftBlockFP, blocks, lastSplitValues, negativeDeltas, false,
