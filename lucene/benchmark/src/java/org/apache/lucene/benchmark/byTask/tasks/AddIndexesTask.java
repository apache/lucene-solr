...
/**
 * Adds an input index to an existing index, using {@link IndexWriter#addIndexes(Directory...)} or
 * {@link IndexWriter#addIndexes(CodecReader...)}. The location of the input index is specified by
 * the parameter {@link #ADDINDEXES_INPUT_DIR} and is assumed to be a directory on the file system.
 *
 * <p>Takes optional parameter {@code useAddIndexesDir} which specifies which addIndexes variant to
...
public class AddIndexesTask extends PerfTask {
  ...
  public void setup() throws Exception {
    ...
    if (inputDirProp == null) {
      throw new IllegalArgumentException(
          "config parameter " + ADDINDEXES_INPUT_DIR + " not specified in configuration");
    }
   ...
   *
   * @param params {@code useAddIndexesDir=true} for using {@link
   *     IndexWriter#addIndexes(Directory...)} or {@code false} for using {@link
   *     IndexWriter#addIndexes(CodecReader...)}. Defaults to {@code true}.
   */
  @Override
