...
public class TestDropIfFlaggedFilter extends BaseTokenStreamTestCase {
  ...
  public void testDroppedFirst() throws Exception {
    ...
    String randTok = String.valueOf(UUID.randomUUID());

    TokenStream ts = new CannedTokenStream(token, token2, token3, token4);
    ts = new DropIfFlaggedFilter(ts, 4) { // 100
      @Override
    ...
    assertTokenStreamContents(ts, new String[]{
        randTok, "bar", "baz"}, new int[]{0, 4, 8}, new int[]{2, 6, 10}, new int[]{1, 1, 1});
  }
  ...
  public void testFirstNotPos0() throws Exception {
    ...
    assertTokenStreamContents(ts, new String[]{randTok, "bar", "baz"},
        new int[]{1, 5, 9}, new int[]{3, 7, 11}, new int[]{1, 1, 1});
  }
}
