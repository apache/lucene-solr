...
public class TestNGramFilters extends BaseTokenStreamFactoryTestCase {
  ...
  public void testNGramFilter3() throws Exception {
    ...
    stream =
        tokenFilterFactory(
                "NGram", "minGramSize", "2", "maxGramSize", "3", "preserveOriginal", "true")
            .create(stream);
    assertTokenStreamContents(stream, new String[] {"te", "tes", "es", "est", "st", "test"});
  }
  ...
  public void testEdgeNGramTokenizer2() throws Exception {
    Reader reader = new StringReader("test");
    TokenStream stream =
        tokenizerFactory("EdgeNGram", "minGramSize", "1", "maxGramSize", "2").create();
    ((Tokenizer) stream).setReader(reader);
  ...
  public void testEdgeNGramFilter3() throws Exception {
    ...
    stream =
        tokenFilterFactory(
                "EdgeNGram", "minGramSize", "1", "maxGramSize", "2", "preserveOriginal", "true")
            .create(stream);
    assertTokenStreamContents(stream, new String[] {"t", "te", "test"});
  }
  ...
  public void testBogusArguments() throws Exception {
    ...
    expected =
        expectThrows(
            ...
            () -> {
              tokenFilterFactory(
                  "NGram", "minGramSize", "2", "maxGramSize", "5", "bogusArg", "bogusValue");
            });
    assertTrue(expected.getMessage().contains("Unknown parameters"));
    ...
    expected =
        expectThrows(
            ...
            () -> {
              tokenFilterFactory(
                  "EdgeNGram", "minGramSize", "2", "maxGramSize", "5", "bogusArg", "bogusValue");
            });
    assertTrue(expected.getMessage().contains("Unknown parameters"));
  }
}
