...
public class TestConcatenatingTokenStream extends BaseTokenStreamTestCase {
  ...
  public void testOffsetGaps() throws IOException {
    CannedTokenStream cts1 =
        new CannedTokenStream(2, 10, new Token("a", 0, 1), new Token("b", 2, 3));
    CannedTokenStream cts2 =
        new CannedTokenStream(2, 10, new Token("c", 0, 1), new Token("d", 2, 3));
  ...
  public void testInconsistentAttributeFactories() throws IOException {
    ...
    final MockTokenizer first =
        new MockTokenizer(
            AttributeFactory.DEFAULT_ATTRIBUTE_FACTORY, MockTokenizer.WHITESPACE, true);
    final MockTokenizer second =
        new MockTokenizer(
            TokenStream.DEFAULT_TOKEN_ATTRIBUTE_FACTORY, MockTokenizer.WHITESPACE, true);
