...
public class TestSegmentingTokenizerBase extends BaseTokenStreamTestCase {
  ...
  public void testBasics() throws IOException {
    assertAnalyzesTo(
        sentence,
        "The acronym for United States is U.S. but this doesn't end a sentence",
  ...
  static class WholeSentenceTokenizer extends SegmentingTokenizerBase {
    ...
    protected boolean incrementWord() {
      if (hasSentence) {
        ...
        offsetAtt.setOffset(
            correctOffset(offset + sentenceStart), correctOffset(offset + sentenceEnd));
        return true;
      } else {
        return false;
      }
  ...
  /**
   * simple tokenizer, that bumps posinc + 1 for tokens after a sentence boundary to inhibit phrase
   * queries without slop.
   */
  static class SentenceAndWordTokenizer extends SegmentingTokenizerBase {
    int sentenceStart, sentenceEnd;
    int wordStart, wordEnd;
    int posBoost =
        -1; // initially set to -1 so the first word in the document doesn't get a pos boost
    ...
    protected boolean incrementWord() {
      ...
      while (wordStart < sentenceEnd) {
        if (Character.isLetterOrDigit(buffer[wordStart])) break;
        wordStart++;
      }
