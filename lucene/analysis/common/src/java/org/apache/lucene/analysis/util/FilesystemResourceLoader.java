...
/**
 * Simple {@link ResourceLoader} that opens resource files from the local file system, optionally
 * resolving against a base directory.
 *
 * <p>This loader wraps a delegate {@link ResourceLoader} that is used to resolve all files, the
 * current base directory does not contain. {@link #newInstance} is always resolved against the
 * delegate, as a {@link ClassLoader} is needed.
 *
 * <p>You can chain several {@code FilesystemResourceLoader}s to allow lookup of files in more than
 * one base directory.
 */
