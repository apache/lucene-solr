...
/**
  ...
  public int stem(char s[], int len) {
    if(useEnhancedStemmer){
    	...
    	boolean foundPrefixRule = enhancedStemPrefix(str, len);
    	if(validated){
    		return str.length;
    	}
    	boolean foundSuffixRule = enhancedStemSuffix(str, str.length);
    	if(foundSuffixRule){
    		return str.length;
    	}
    	boolean foundInfixRule = enhancedStemInfix(str, str.length);
    	if(foundInfixRule){
    		return str.length;
    	}
  ...
  /**
   * Stem a prefix off an Arabic word using enhanced stemmer.
   * @param s input buffer
   * @param len length of input buffer
   * @return true if Prefix is found and false if there is no matched Prefix.
   */
  public boolean enhancedStemPrefix(char s[], int len){
	validated = false;
	...
	for (int i = 0; i < singlePrefixes.length; i++){ 
      if (startsWithCheckLength(s, len, singlePrefixes[i])){
    	...
    	boolean foundSuffixRule = enhancedStemSuffix(s2, s2.length);
    	...
    	boolean foundInfixRule = enhancedStemInfix(s2, s2.length);
  ...
  /**
   * Stem a suffix off an Arabic word using enhanced stemmer.
   * @param s input buffer
   * @param len length of input buffer
   * @return true if Suffix is found and false if there is no matched Suffix.
   */
  public boolean enhancedStemSuffix(char s[], int len){
	validated = false;
   ...
   //Check for suffix set 2 in order
	for (int i = 0; i < suffixes_set2.length; i++){ 
      if (endsWithCheckLength(s, len, suffixes_set2[i])){
    	  ...
    	  /*
    	   * This will skip many valid suffix removal, while in these exceptional patterns 
    	   * the suffix will not be removed as it is validated before removal.
    	  //Check the exceptional patterns of suffix removal
    	  if(suffixes_set2[i].length == 1 &&
    		 ((suffixes_set2[i][0] == YEH && (s[0] == ALEF || s[0] == YEH || s[0] == TEH))
      	       || (suffixes_set2[i][0] == HEH && (s[0] == ALEF || s[len-3] == ALEF)) => this is not correct for some cases like falaha && amira 
    		   || (suffixes_set2[i][0] == ALEF && (s[len-1] == ALEF && s[len-2] == YEH && s[len-3] == ALEF && s[1] != WAW))
    		 )){
    		 continue; 
    	  }*/
    	  ...
    	  if(validatedStemList.contains(s, 0, len - suffixes_set2[i].length)){
    		  ...
    		  if(s[len - suffixes_set2[i].length -1] == TEH){
	    		  ...
	    		  if(validatedStemList.contains(s, 0, len - suffixes_set2[i].length)){
	    			  ...
	    			  boolean foundInfixRule = enhancedStemInfix(s, len - suffixes_set2[i].length);
		    		  if(foundInfixRule){
		    			  return foundSuffixRule;
		    		  }
		    		  //reset HEH to TEH
    		  ...
    		  if( (i == 0 /*Heh Alef*/|| i == 6 /*Heh Mem*/|| i == 8 /*Heh*/) && (len > suffixes_set2[i].length +2) &&
    			s[len - suffixes_set2[i].length -1] == YEH_HAMZA && s[len - suffixes_set2[i].length -2] == ALEF){
	    		  ...
	    		  //s[len - suffixes_set2[i].length -1] = YEH_HAMZA;
    		  }
    		  
    		  //Commented as it wrongly stemmed Arabic words like azan, atnen, akon
    		  //In case of plural suffixes, stem it even it is not validated
//        	  if(i == 1 /*Waw Noon*/|| i == 2 /*Waw Alef*/|| i == 3 /*Yeh Noon*/|| i == 4 /*Alef Noon*/){ 
//        		  str = Arrays.copyOfRange(s, 0, len - suffixes_set2[i].length);
//        		  return foundSuffixRule;
//        	  }
    	  ...
    	  for (int j = 0; j < suffixes_set3.length; j++){ 
    	      if (endsWithCheckLength(s2, s2.length, suffixes_set3[j])){
    	    	  if(validatedStemList.contains(s2, 0, s2.length - suffixes_set3[j].length)){
    	    		...
    	    		//Try to stem Infix and check again    		  
    	    		boolean foundInfixRule =enhancedStemInfix(s2, s2.length - suffixes_set3[j].length);
    	    		if(foundInfixRule){
    	    		    return foundSuffixRule;
    	    		}
    	  ...
    	  if(suffixes_set2[i].length == 1 && suffixes_set2[i][0] == ALEF 
	    	  ...
	    	  if(validatedStemList.contains(s2, 0, s2.length - 1)){
	    		...
	    		//Try to stem Infix and check again    		  
	    		boolean  foundInfixRule = enhancedStemInfix(s2, s2.length - 1);
	    		if(foundInfixRule){
	    		    return foundSuffixRule;
	    		}
	    		if(!highAccuracy){
  ...
  /**
   * Stem a infix off an Arabic word using enhanced stemmer.
   * @param s input buffer
   * @param len length of input buffer
   * @return true if Infix rule found and false if no matched Infix rule.
   */
  public boolean enhancedStemInfix(char s[], int len){
  ...
  /**
   * Stem a infix off an Arabic noun using enhanced stemmer.
   * @param s input buffer
   * @param len length of input buffer
   * @return true if Infix rule found and false if no matched Infix rule.
   */
  ...
  /**
   * Stem a infix of an Arabic verb using enhanced stemmer.
   * @param s input buffer
   * @param len length of input buffer
   * @return true if Infix rule found and false if no matched Infix rule.
   */
