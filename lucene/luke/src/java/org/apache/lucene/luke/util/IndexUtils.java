...
public final class IndexUtils {
   ...
   */
  public static boolean isLive(IndexReader reader, int docid) {
    if (reader instanceof LeafReader) {
      Bits liveDocs = ((LeafReader) reader).getLiveDocs();
      ...
      for (LeafReaderContext ctx : reader.leaves()) {
        ...
        if (liveDocs != null) {
          allLive = false;
          live = live || liveDocs.get(docid);
        }
      }
      // return true if there are no deleted documents or the docid is included in any segments.
      return allLive || live;
    }
  }
