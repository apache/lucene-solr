...
public class Geo3dDistanceCalculator implements DistanceCalculator {
  ...
  //@Override
  public Point pointOnBearing2(Point from, double distDEG, double bearingDEG, SpatialContext ctx, Point reuse) {
    ...
    double uSq = cosSqα * (planetModel.ab* planetModel.ab - planetModel.c*planetModel.c) / (planetModel.c*planetModel.c);
    ...
    double σ = distDEG; //s / (planetModel.c*A);
    double σʹ;
    double iterations = 0;
    do {
    ...
    if (iterations>=200) {
      throw new RuntimeException("Formula failed to converge"); // not possible?
    }
    ...
    return ctx.getShapeFactory().pointXY(φ2, λ2);
  }
