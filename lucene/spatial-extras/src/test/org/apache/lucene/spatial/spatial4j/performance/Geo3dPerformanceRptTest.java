...
public abstract class Geo3dPerformanceRptTest extends SpatialTestCase {
  ...
  protected RecursivePrefixTreeStrategy quadRecursiveStrategy;
  ...
  protected CompositeSpatialStrategy quadStrategy;
  ...
  public void setUp() throws Exception {
    ...
    helper = new QuadPrefixTree(ctx, QuadPrefixTree.MAX_LEVELS_POSSIBLE);
    grid = new QuadPrefixTree(ctx, helper.getLevelForDistance(precision()));
    quadRecursiveStrategy = new RecursivePrefixTreeStrategy(grid,
        getClass().getSimpleName() + "_rpt");
    quadRecursiveStrategy.setDistErrPct(distErrPct());
    quadRecursiveStrategy.setPointsOnly(pointsOnly());
    serializedDVStrategy = new SerializedDVStrategy(ctx, getClass().getSimpleName() + "_sdv");
    this.quadStrategy = new CompositeSpatialStrategy("composite_" + getClass().getSimpleName(),
        quadRecursiveStrategy, serializedDVStrategy);
  ...
  public void testOperations() throws IOException {
    ...
    long geoHashHitsRecursive  = executeQueries("geohash recursive", geohashStrategy.getIndexStrategy());
    long geoHashHits = executeQueries("geohash composite", geohashStrategy);
    ...
    testLoad("quad", quadStrategy);
    long quadHitsRecursive  = executeQueries("quad recursive", quadStrategy.getIndexStrategy());
    long quadHits = executeQueries("quad composite", quadStrategy);
    ...
    testLoad("s2 arity 4", s2Strategy1);
    long s2Hits1Recursive = executeQueries("s2 arity 4 recursive", s2Strategy1.getIndexStrategy());
    long s2Hits1 = executeQueries("s2 arity 4 composite", s2Strategy1);

    indexWriter.deleteAll();
    indexWriter.forceMergeDeletes(true);
    System.out.println();

    testLoad("s2 arity 16", s2Strategy2);
    long s2Hits2Recursive = executeQueries("s2 arity 16 recursive", s2Strategy2.getIndexStrategy());
    long s2Hits2 = executeQueries("s2 arity 16 composite", s2Strategy2);
    ...
    testLoad("s2 arity 64", s2Strategy3);
    long s2Hits3Recursive = executeQueries("s2 arity 64 recursive", s2Strategy3.getIndexStrategy());
    long s2Hits3 = executeQueries("s2 arity 64 composite", s2Strategy3);
    ...
    assertEquals(geoHashHits, quadHits);
  }

  protected void testLoad(String tree, SpatialStrategy strategy) throws IOException {
  ...
  protected long executeQueries(String tree ,SpatialStrategy strategy) {
    long start = System.currentTimeMillis();
    long totalHits = 0;
  ...
  protected Document newDoc(String id, Shape shape, SpatialStrategy strategy) {
