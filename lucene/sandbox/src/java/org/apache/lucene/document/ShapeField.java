...
public final class ShapeField {
  ...
  /**
   * A line is encoded using 4 points and an extra point with encoded information in four bits of how to reconstruct it.
   ...
   */
  public static void decodeTriangle(byte[] t, DecodedTriangle triangle, Version version) {
    final int bits = NumericUtils.sortableBytesToInt(t, 6 * BYTES);
    if (version.onOrAfter(Version.LUCENE_8_5_0)) {
      int tCode = (((1 << 4) - 1) & (bits >> 0));
      if (tCode < 8) {
        triangle.type = DecodedTriangle.TYPE.TRIANGLE;
        decodeTriangle(t, triangle, bits, tCode);
      } else if (tCode == 8) {
        triangle.type = DecodedTriangle.TYPE.POINT;
        decodePoint(t,  triangle);
      } else {
        triangle.type = DecodedTriangle.TYPE.LINE;
        decodeLine(t, triangle, bits, tCode);
      }
    } else {
      decodePre85Triangle(t, triangle, bits);
    }
  }
  ...
  private static void decodeLine(final byte[] t, final DecodedTriangle triangle, final int bits, final int tCode) {
    assert tCode == LINE_MINY_MINX_MAXY_MAXX || tCode == LINE_MAXY_MINX_MINY_MAXX;
    switch (tCode) {
      case LINE_MINY_MINX_MAXY_MAXX:
        triangle.aY = NumericUtils.sortableBytesToInt(t, 0 * BYTES);
        triangle.aX = NumericUtils.sortableBytesToInt(t, 1 * BYTES);
    ...
    triangle.ab = (bits & 1 << 4) == 1 << 4;
    triangle.bc = (bits & 1 << 5) == 1 << 5;
    triangle.ca = (bits & 1 << 6) == 1 << 6;
  }

  private static void decodeTriangle(final byte[] t, final DecodedTriangle triangle, final int bits, int tCode) {
    switch (tCode) {
  ...
  /**
   * This is the way of encoding before Lucene 8.5 (For testing backwards compatibility).
   *
   * A triangle is encoded using 6 points and an extra point with encoded information in three bits of how to reconstruct it.
   * Triangles are encoded with CCW orientation and might be rotated to limit the number of possible reconstructions to 2^3.
   * Reconstruction always happens from west to east.
   */
  protected static void encodePre85Triangle(byte[] bytes, int aLat, int aLon, boolean abFromShape, int bLat, int bLon, boolean bcFromShape, int cLat, int cLon, boolean caFromShape) {
    assert bytes.length == 7 * BYTES;
    int aX;
    int bX;
    int cX;
  ...
  private static void decodePre85Triangle(final byte[] t, final DecodedTriangle triangle, final int bits) {
  ...
  /**
   * Represents a encoded triangle using {@link ShapeField#decodeTriangle(byte[], DecodedTriangle, Version)}.
   */
