...
/**
 * A helper class for {@link IntervalQuery} that provides an {@link IntervalIterator} for a given
 * field and segment
 *
...
public abstract class IntervalsSource {
  ...
  /**
   * Create an {@link IntervalIterator} exposing the minimum intervals defined by this {@link
   * IntervalsSource}
   *
   * <p>Returns {@code null} if no intervals for this field exist in this segment
   *
   * @param field the field to read positions from
   * @param ctx the context for which to return the iterator
   */
  public abstract IntervalIterator intervals(String field, LeafReaderContext ctx)
      throws IOException;

  /**
   * Return a {@link MatchesIterator} over the intervals defined by this {@link IntervalsSource} for
   * a given document and field
   *
   * <p>Returns {@code null} if no intervals exist in the given document and field
   *
   * @param field the field to read positions from
   * @param ctx the document's context
   * @param doc the document to return matches for
   */
  public abstract IntervalMatchesIterator matches(String field, LeafReaderContext ctx, int doc)
      throws IOException;
   ...
   *
   * <p>Most implementations can return {@code Collections.singleton(this)}
   */
