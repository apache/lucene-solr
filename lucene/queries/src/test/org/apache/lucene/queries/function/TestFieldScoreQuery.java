 ...
 *
 * <p>Tests here create an index with a few documents, each having an int value indexed field and a
 * float value indexed field. The values of these fields are later used for scoring.
 *
 * <p>The rank tests use Hits to verify that docs are ordered (by score) as expected.
 *
 * <p>The exact score tests use TopDocs top to verify the exact score.
 */
public class TestFieldScoreQuery extends FunctionTestSetup {
  ...
  private void doTestRank(ValueSource valueSource) throws Exception {
    ...
    for (int i = 0; i < h.length; i++) {
      ...
      assertTrue(
          "res id " + resID + " should be < prev res id " + prevID, resID.compareTo(prevID) < 0);
      prevID = resID;
    }
    r.close();
  }
  ...
  protected Query getFunctionQuery(ValueSource valueSource) {
    if (random().nextBoolean()) {
      ...
      return new FunctionRangeQuery(
          valueSource, lower, upper, true, true); // will match all docs based on the indexed data
    }
  }
}
