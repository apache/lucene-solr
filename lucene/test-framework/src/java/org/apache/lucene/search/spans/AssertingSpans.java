...
class AssertingSpans extends Spans {
  ...
  static enum State {
    ...
    /**
     * two-phase iterator has moved to a new docid, but {@link TwoPhaseIterator#matches()} has not
     * been called or it returned false (so you should not do things with the enum)
     */
    DOC_UNVERIFIED,

    /**
     * iterator set to a valid docID, but position iteration has not yet begun ({@link
     * #startPosition() == -1})
     */
    POS_START,

    /**
     * iterator set to a valid docID, and positioned (-1 < {@link #startPosition()} < {@link
     * #NO_MORE_POSITIONS})
     */
    ITERATING,
  ...
  public int docID() {
    int doc = in.docID();
    assert doc == this.doc
        : "broken docID() impl: docID() = "
            + doc
            + ", but next/advance last returned: "
            + this.doc
            + ": "
            + in;
    return doc;
  ...
  class AssertingTwoPhaseView extends TwoPhaseIterator {
    ...
    public float matchCost() {
      ...
      if (Float.isNaN(cost)) {
        throw new AssertionError(
            "matchCost()=" + cost + " should not be NaN on doc ID " + approximation.docID());
      }
      if (cost < 0) {
        throw new AssertionError(
            "matchCost()=" + cost + " should be non negative on doc ID " + approximation.docID());
      }
      return cost;
    }
  }
