...
public class TestDirectoryTaxonomyWriter extends FacetTestCase {
  ...
  public void testCommit() throws Exception {
    ...
    assertEquals(
        "No categories should have been committed to the underlying directory", 1, r.numDocs());
    r.close();
    ltw.close();
    dir.close();
  }
  ...
  public void testCommitUserData() throws Exception {
    ...
    assertEquals(
        "2 categories plus root should have been committed to the underlying directory",
        3,
        r.numDocs());
    Map<String, String> readUserCommitData = r.getIndexCommit().getUserData();
    assertTrue(
        "wrong value extracted from commit data",
        "1 2 3".equals(readUserCommitData.get("testing")));
    assertNotNull(
        DirectoryTaxonomyWriter.INDEX_EPOCH + " not found in commitData",
    ...
    assertNotNull(
        DirectoryTaxonomyWriter.INDEX_EPOCH + " not found in taoxWriter.commitData",
        data.get(DirectoryTaxonomyWriter.INDEX_EPOCH));
    taxoWriter.close();
  ...
  public void testReplaceTaxonomy() throws Exception {
    ...
    assertEquals(
        "category 'a' received new ordinal?", ordA, taxoWriter.addCategory(new FacetLabel("a")));
    ...
    assertEquals(
        "ordinal should have been 2 since only one category was added by replaceTaxonomy",
        2,
        newOrdB);
  ...
  public void testReaderFreshness() throws Exception {
    ...
    Directory dir = newDirectory();
    DirectoryTaxonomyWriter taxoWriter =
        new DirectoryTaxonomyWriter(dir, OpenMode.CREATE, NO_OP_CACHE);
