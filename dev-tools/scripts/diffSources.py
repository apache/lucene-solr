...
while True:
  ...
  if l.startswith('diff ') or l.startswith('Binary files '):
    keep = not l.endswith('timehints.txt') and l.lower().find('/build/') == -1 and l.lower().find('/eclipse-build/') == -1 and (l.lower().startswith('Only in') or ((l.lower().endswith('.java') or l.lower().endswith('.txt') or l.lower().endswith('.xml') or l.lower().endswith('.html') or l.lower().endswith('.iml') or l.lower().find('/meta-inf/services/') != -1) and l.find('/.svn/') == -1))
    if keep:
      print
      print
      print l.strip()
